...
interface MTGPriceLookupProps {
  ...
  onPriceLoad?: (data: {
    ...
    /**
     * ADDITIVE (card detail V2, MTG adapter). The same three-number range
...
export function MTGPriceLookup({ card, dcmGrade, isOwner = false, guardIdentity, onPriceLoad }: MTGPriceLookupProps) {
  ...
  const getVariant = () => {
    ...
    if (card.rarity_or_variant) {
      ...
      if (variantLower.includes('borderless')) return 'Borderless';
      if (variantLower.includes('extended art')) return 'Extended Art';
      if (variantLower.includes('showcase')) return 'Showcase';
      if (variantLower.includes('retro frame')) return 'Retro Frame';
  ...
  const fetchPrices = async (overrideProductId?: string, forceRefresh: boolean = false) => {
    ...
    if (productIdToUse) {
      console.log('[MTGPriceLookup] === FETCHING MANUAL SELECTION ===');
      console.log('[MTGPriceLookup] Product ID:', productIdToUse);
      try {
    ...
    console.log('[MTGPriceLookup] === SEARCH ===');
    console.log('[MTGPriceLookup] Card:', cardName);
    console.log('[MTGPriceLookup] Set:', card.set_name);
    console.log('[MTGPriceLookup] Collector #:', card.collector_number);
    console.log('[MTGPriceLookup] Foil:', card.is_foil || false);
    console.log('[MTGPriceLookup] Variant:', variant || '(none)');
  ...
  const fetchVariants = async () => {
    ...
    try {
      ...
      console.error('[MTGPriceLookup] Error fetching variants:', err);
    } finally {
  ...
  const saveSelection = async (productId: string, productName: string) => {
    ...
    try {
      ...
      console.error('[MTGPriceLookup] Error saving selection:', err);
    } finally {
  ...
  const clearSelection = async () => {
    ...
    try {
      ...
      console.error('[MTGPriceLookup] Error clearing selection:', err);
    } finally {
      setSavingSelection(false);
    }
  };
  ...
  const getPriceChartingUrl = () => {
    if (!priceData) return 'https://www.pricecharting.com/category/magic-the-gathering';
