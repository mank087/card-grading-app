...
interface NormalizedOnePiecePrices {
  raw: number | null;
...
interface OnePiecePricingResult {
  success: boolean;
...
interface OnePiecePriceLookupProps {
  card: {
    id?: string;
    card_name?: string;
    set_name?: string;
...
export function OnePiecePriceLookup({ card, dcmGrade, isOwner = false, guardIdentity, onPriceLoad }: OnePiecePriceLookupProps) {
  const [priceData, setPriceData] = useState<OnePiecePricingResult['data'] | null>(null);
  ...
  // One Piece variants: parallel, parallel_manga, alternate_art, sp, manga, promo, special_art
  const getVariant = () => {
    if (card.rarity_or_variant) {
      const variantLower = card.rarity_or_variant.toLowerCase().replace(/_/g, ' ');
      // Check for parallel first (includes parallel_manga)
      if (variantLower.includes('parallel')) return 'Parallel';
      // Alt art / alternate art
      if (variantLower.includes('alt art') || variantLower.includes('alternate art')) return 'Alt Art';
      // Manga art
      if (variantLower.includes('manga')) return 'Manga Art';
      // SP (Special Parallel) - treated as a special variant
      if (variantLower === 'sp' || variantLower.includes('special parallel')) return 'SP';
      // Special art
      if (variantLower.includes('special art')) return 'Special Art';
      // Promo
  ...
  const refetchAfterStalePrice = async () => {
    ...
    console.log('[OnePiecePriceLookup] Card changed while pricing; refetching once');
    await fetchPrices(undefined, true);
  };

  // Save estimated price to database so collection/portfolio pages reflect the latest value
  const savePriceEstimate = async (data: OnePiecePricingResult['data']) => {
    ...
    try {
      ...
      console.error('[OnePiecePriceLookup] Error saving price estimate:', err);
    }
  };
  ...
  const fetchPrices = async (overrideProductId?: string, forceRefresh: boolean = false) => {
    ...
    if (productIdToUse) {
      console.log('[OnePiecePriceLookup] === FETCHING MANUAL SELECTION ===');
      console.log('[OnePiecePriceLookup] Product ID:', productIdToUse);
      try {
        ...
        const data: OnePiecePricingResult = await response.json();
        ...
        console.error('[OnePiecePriceLookup] Error fetching selected product:', err);
    ...
    console.log('[OnePiecePriceLookup] === SEARCH ===');
    console.log('[OnePiecePriceLookup] Card:', cardName);
    console.log('[OnePiecePriceLookup] Set:', card.set_name);
    console.log('[OnePiecePriceLookup] Collector #:', card.collector_number);
    console.log('[OnePiecePriceLookup] Variant:', variant || '(none)');
    ...
    try {
      ...
      console.error('[OnePiecePriceLookup] Error:', err);
      setError(err instanceof Error ? err.message : 'Failed to fetch prices');
    } finally {
      setLoading(false);
    }
  ...
  const fetchVariants = async () => {
    ...
    try {
      ...
      console.error('[OnePiecePriceLookup] Error fetching variants:', err);
    } finally {
  ...
  const saveSelection = async (productId: string, productName: string) => {
    ...
    try {
      ...
      console.error('[OnePiecePriceLookup] Error saving selection:', err);
    } finally {
  ...
  const clearSelection = async () => {
    ...
    try {
      ...
      console.error('[OnePiecePriceLookup] Error clearing selection:', err);
    } finally {
      setSavingSelection(false);
    }
  };
  ...
  }, [card.card_name, card.set_name, card.collector_number, dcmGrade, card.dcm_selected_product_id]);
  ...
  const getPriceChartingUrl = () => {
    if (!priceData) return 'https://www.pricecharting.com/category/one-piece-card-game';
            ...
            <details className="mt-4 border-t border-gray-100 pt-3 group" open={priceData.matchConfidence === 'low' || priceData.matchConfidence === 'medium'}>
              <summary className={`text-xs cursor-pointer font-medium flex items-center gap-1 list-none [&::-webkit-details-marker]:hidden ${
                ...
                {priceData.matchConfidence === 'low' || priceData.matchConfidence === 'medium'
                  ...
                  : 'See other card variants (Parallel, Alt Art, etc.)'}
              </summary>
