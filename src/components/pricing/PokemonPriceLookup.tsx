...
interface NormalizedPokemonPrices {
  raw: number | null;
...
interface PokemonPricingResult {
  success: boolean;
...
interface PokemonPriceLookupProps {
  card: {
    id?: string;
    player_or_character?: string;  // Pokemon name
    card_name?: string;            // Card name (may include Pokemon + type)
    year?: string;
    set_name?: string;
    card_number?: string;
    rarity_or_variant?: string;    // Variant like "1st Edition", "Holofoil"
    holofoil?: boolean | string;
...
export function PokemonPriceLookup({ card, dcmGrade, isOwner = false, guardIdentity, onPriceLoad }: PokemonPriceLookupProps) {
  const [priceData, setPriceData] = useState<PokemonPricingResult['data'] | null>(null);
  ...
  // Get Pokemon name from available fields
  const getPokemonName = () => {
    return card.player_or_character || card.card_name || '';
  };
  ...
  const getVariant = () => {
    if (card.rarity_or_variant) {
      ...
      if (variantLower.includes('1st edition') || variantLower.includes('first edition')) return '1st Edition';
      if (variantLower.includes('shadowless')) return 'Shadowless';
      if (variantLower.includes('reverse holo')) return 'Reverse Holo';
      if (variantLower.includes('holo') || variantLower.includes('holofoil')) return 'Holofoil';
      return card.rarity_or_variant;
    }
    if (card.first_edition) return '1st Edition';
    if (card.reverse_holo) return 'Reverse Holo';
    if (card.holofoil === true || card.holofoil === 'true') return 'Holofoil';
    return undefined;
  };
  ...
  const refetchAfterStalePrice = async () => {
    ...
    console.log('[PokemonPriceLookup] Card changed while pricing; refetching once');
    await fetchPrices(undefined, true);
  };

  // Save estimated price to database so collection/portfolio pages reflect the latest value
  const savePriceEstimate = async (data: PokemonPricingResult['data']) => {
    ...
    try {
      ...
      console.error('[PokemonPriceLookup] Error saving price estimate:', err);
    }
  };
  ...
  const fetchPrices = async (overrideProductId?: string, forceRefresh: boolean = false) => {
    ...
    if (productIdToUse) {
      console.log('[PokemonPriceLookup] === FETCHING MANUAL SELECTION ===');
      console.log('[PokemonPriceLookup] Product ID:', productIdToUse);
      try {
        ...
        const data: PokemonPricingResult = await response.json();
        ...
        console.error('[PokemonPriceLookup] Error fetching selected product:', err);
    ...
    console.log('[PokemonPriceLookup] === SEARCH ===');
    console.log('[PokemonPriceLookup] Pokemon:', pokemonName);
    console.log('[PokemonPriceLookup] Year:', card.year);
    console.log('[PokemonPriceLookup] Set:', card.set_name);
    console.log('[PokemonPriceLookup] Card #:', card.card_number);
    console.log('[PokemonPriceLookup] Variant:', variant || '(none)');
    ...
    try {
      const response = await fetch('/api/pricing/pokemon', {
        ...
        body: JSON.stringify({
          ...
          variant,
          isHolo: card.holofoil === true || card.holofoil === 'true',
          isFirstEdition: card.first_edition,
          isReverseHolo: card.reverse_holo,
          dcmGrade,
      ...
      console.error('[PokemonPriceLookup] Error:', err);
      setError(err instanceof Error ? err.message : 'Failed to fetch prices');
    } finally {
      setLoading(false);
    }
  ...
  const fetchVariants = async () => {
    if (!getPokemonName() || availableVariants.length > 0) {
    ...
    try {
      ...
      console.error('[PokemonPriceLookup] Error fetching variants:', err);
    } finally {
  ...
  const saveSelection = async (productId: string, productName: string) => {
    ...
    try {
      ...
      console.error('[PokemonPriceLookup] Error saving selection:', err);
    } finally {
  ...
  const clearSelection = async () => {
    ...
    try {
      ...
      console.error('[PokemonPriceLookup] Error clearing selection:', err);
    } finally {
      setSavingSelection(false);
    }
  };
  ...
  }, [card.player_or_character, card.card_name, card.year, card.set_name, card.rarity_or_variant, dcmGrade, card.dcm_selected_product_id]);
  ...
  const getPriceChartingUrl = () => {
    if (!priceData) return 'https://www.pricecharting.com/category/pokemon-cards';
    ...
    return `https://www.pricecharting.com/search-products?q=${encodeURIComponent(priceData.queryUsed || getPokemonName())}`;
  };

  // Get market range
  const getMarketRange = () => computeMarketRange(priceData?.prices);
            ...
            <details className="mt-4 border-t border-gray-100 pt-3 group" open={priceData.matchConfidence === 'low' || priceData.matchConfidence === 'medium'}>
              <summary className={`text-xs cursor-pointer font-medium flex items-center gap-1 list-none [&::-webkit-details-marker]:hidden ${
                ...
                {priceData.matchConfidence === 'low' || priceData.matchConfidence === 'medium'
                  ...
                  : 'See other card variants (1st Edition, Holofoil, etc.)'}
              </summary>
