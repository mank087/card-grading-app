...
export const EbayListingModal: React.FC<EbayListingModalProps> = ({
  ...
  const createListing = async () => {
    ...
    try {
      ...
      console.log('[eBay Listing] Creating listing for card:', card.id);
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto p-6">
          ...
          {step === 'error' && (
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-gray-600 text-center mb-4 whitespace-pre-line max-w-md">{error}</p>
              {error?.includes('eBay.com') && (
                <a
          ...
          {ebayConnectionStatus === 'connected' && ebayUsername && step !== 'success' && step !== 'publishing' && (
            <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-100">
              ...
              {/* Disconnecting is an ACCOUNT action, not a step in publishing
                  one listing — it lives on the InstaList Settings tab. This
                  link only takes the person there. */}
              <a
                href="/instalist-marketplace?tab=settings"
