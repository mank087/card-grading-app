...
export function EbayPriceLookup({ card, cardId, category = 'sports' }: EbayPriceLookupProps) {
  ...
  // Auto-fetch on mount - use cached prices if available
  useEffect(() => {
      ...
      <div className="flex items-center justify-between gap-2 mb-4">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="text-base sm:text-lg font-bold text-gray-800">eBay Market Prices</h3>
      ...
      {priceData && (
        <>
          ...
          {priceData.items.length > 0 && (
            <div className="mb-4">
              <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${getPriceConfidence(priceData.items.length).color}`}>
                <svg className="w-3 h-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  {getPriceConfidence(priceData.items.length).level === 'high' ? (
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                  ) : getPriceConfidence(priceData.items.length).level === 'medium' ? (
                    <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  ) : (
                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  )}
                </svg>
                <span className="whitespace-nowrap">{getPriceConfidence(priceData.items.length).label}</span>
              </div>
            </div>
          )}

          {/* Price Distribution Chart - Always visible */}
          {priceData.items.length > 0 && (
