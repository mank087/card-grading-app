...
export function MTGCardDetails() {
  ...
  const fetchMTGCardDetails = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/lorcana/${cardId}?t=${Date.now()}`); // Cache-busting
        ...
        if (res.status === 429) {
          ...
          const retryWithBackoff = async (attempt: number = 1): Promise<void> => {
            ...
            try {
              ...
              const retryRes = await fetch(`/api/lorcana/${cardId}?t=${Date.now()}`); // Cache-busting
