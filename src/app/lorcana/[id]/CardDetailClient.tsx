  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'Lorcana Cards',
        item: 'https://dcmgrading.com/upload?category=Lorcana'
      },
      {
        '@type': 'ListItem',
        position: 3,
...
export function MTGCardDetails() {
  ...
  const classicQrDataUrl = useClassicQrDataUrl(
    isClassicLabel && origin && cardId ? `${origin}/lorcana/${cardId}` : null
  );
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-6">
              ...
              {(() => {
                ...
                return (
                  <div id="tour-download-buttons" className="flex flex-col md:flex-row items-center justify-between gap-4 my-6 px-4">
                    ...
                    {isOwner && <DownloadReportButton card={card} cardType="lorcana" showFounderEmblem={showFounderEmblem} showVipEmblem={showVipEmblem} showCardLoversEmblem={showCardLoversEmblem} labelStyle={labelStyle} customLabelConfig={activeConfig} />}
          ...
          {(() => {
            ...
            const href = outOfCredits ? '/credits' : '/upload?category=Lorcana';
        ...
        <EditCardLabelModal
          ...
          onSaved={async () => {
            try {
              const session = getStoredSession();
              const userParam = session?.user?.id ? `&user_id=${session.user.id}` : '';
              const res = await fetch(`/api/lorcana/${card.id}?t=${Date.now()}${userParam}`);
              if (res.ok) {
                const data = await res.json();
                setCard(data);
              }
