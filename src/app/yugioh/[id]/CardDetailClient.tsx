...
import { generateOnePieceEbaySearchUrl, generateOnePieceEbaySoldListingsUrl, type CardData } from "@/lib/ebayUtils";
...
interface SportsAIGrading {
  ...
  "Grading (DCM Master Scale)"?: {
    ...
    "Visual_Inspection_Results"?: Record<string, any>;
  ...
  // v3.1 top-level fields (legacy ai_grading payload variants)
...
interface SportsCard {
  ...
  // 🏴‍☠️ Yu-Gi-Oh database verification
  yugioh_card_id?: string | null;
  yugioh_reference_image?: string | null;
  yugioh_database_match_confidence?: 'high' | 'medium' | 'low' | null;

  // Yu-Gi-Oh-specific fields
  ygo_card_type?: string | null;     // Leader, Character, Event, Stage
  ygo_attribute?: string | null;    // Red, Blue, Green, Purple, Black, Yellow (or multi-color)
  ygo_atk?: number | null;    // Power value (Characters and Leaders)
  ygo_def?: number | null;     // DON!! cost to play
  ygo_level?: number | null;          // Life value (Leaders only)
  ygo_scale?: number | null;       // Counter value: 1000, 2000, or NULL
  ygo_race?: string | null;     // Combat attribute: Slash, Strike, Ranged, Special, Wisdom
  ygo_archetype?: string | null;     // Character traits/affiliations
  ygo_frame_type?: string | null;  // parallel, manga, parallel_manga, sp, alternate_art, etc.
  ...
  // Legacy grade column (used for GA4 tracking)
  grade?: number | null;

  // Card owner's emblem settings (joined from profile by the API)
  ...
  // Structured conversational grading payloads (dynamic JSON from backend)
  ...
  // DVG image-quality fields
  dvg_image_quality?: string | null;
  dvg_reshoot_required?: boolean | null;

  // Stage 0 detection payload (dynamic JSON)
  stage0_detection?: any;

  // User-reported condition (dual schema — always optional-chain nested access)
  ...
  // Additional card info fields (cross-category DB columns)
  ...
  // Custom label configuration (dynamic JSON)
...
function generateStructuredData(card: any, dvgGrading: any, cardUrl: string) {
  ...
  const productSchema = {
    ...
    category: 'Yu-Gi-Oh! TCG Trading Cards',
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: fullCardName || 'Yu-Gi-Oh Card',
        item: cardUrl
      }
    ]
  };
...
export function YugiohCardDetails() {
  ...
  const classicQrDataUrl = useClassicQrDataUrl(
    isClassicLabel && origin && cardId ? `${origin}/yugioh/${cardId}` : null
  );
  ...
  const heritageQrUrl = origin && cardId ? `${origin}/yugioh/${cardId}` : '';
  useEffect(() => {
  ...
  // Fetch Yu-Gi-Oh Card Details using Yu-Gi-Oh-specific API
  const fetchYugiohCardDetails = useCallback(async () => {
    if (!cardId) return;
    ...
    try {
      ...
      console.log(`[FRONTEND DEBUG] Calling Yu-Gi-Oh API endpoint: /api/yugioh/${cardId}`);
      const res = await fetch(`/api/yugioh/${cardId}?t=${Date.now()}${userParam}`); // Cache-busting
      console.log(`[FRONTEND DEBUG] Yu-Gi-Oh API response status: ${res.status}`);
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) {
          ...
          const retryWithBackoff = async (attempt: number = 1): Promise<void> => {
            ...
            try {
              console.log(`[FRONTEND DEBUG] Retry ${attempt}: Calling /api/yugioh/${cardId}`);
              const retryRes = await fetch(`/api/yugioh/${cardId}?t=${Date.now()}${userParam}`); // Cache-busting
  ...
  useEffect(() => {
    if (cardId) {
      console.log('Fetching Yu-Gi-Oh Card Details for:', cardId);
      fetchYugiohCardDetails();
    }
  }, [cardId, fetchYugiohCardDetails]);
  ...
  useEffect(() => {
    if (card && card.grade && !hasTrackedGradeComplete.current && typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'grade_card_complete', {
        card_category: card.category || 'Yu-Gi-Oh',
        grade: card.grade
      });
  ...
  const regradeCard = async () => {
    ...
    try {
      ...
      queueId = addToQueue({
        cardId: cardId,
        category: 'yugioh',
        categoryLabel: 'Yu-Gi-Oh Card (Re-grade)',
        frontImageUrl: card.front_url,
        status: 'processing',
        resultUrl: `/yugioh/${cardId}`
      });
      console.log('[REGRADE] Added to queue:', queueId);

      // Call Yu-Gi-Oh API with force_regrade parameter to bypass cache
      console.log('[REGRADE] Forcing fresh grading for Yu-Gi-Oh card:', cardId);

      // Re-grading can take 30-60 seconds, so use a longer timeout
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 120000); // 120 second timeout

      const res = await fetch(`/api/yugioh/${cardId}?force_regrade=true&t=${Date.now()}`, {
  ...
  // 🎯 For Yu-Gi-Oh cards: Build dvgGrading from conversational data
  const dvgGrading = {
    ...
    // 🆕 Always include sub_scores from conversational data for Yu-Gi-Oh cards
  ...
  // 🏴‍☠️ ONE PIECE CARD INFO: Read from conversational_card_info
  // Following Pokemon pattern: Database data is merged into card_info by the API route
  ...
  const cardInfo = {
    // Core identification - from conversational_card_info (Database data merged in by API)
    ...
    year: stripMarkdown(convInfo.year) || card.release_date || dvgGrading.card_info?.year,
    manufacturer: stripMarkdown(convInfo.manufacturer) || card.manufacturer_name || 'Konami',
    card_number: stripMarkdown(convInfo.card_number) || stripMarkdown(convInfo.collector_number) || card.card_number || dvgGrading.card_info?.card_number,
    sport_or_category: stripMarkdown(convInfo.sport_or_category) || card.category || dvgGrading.card_info?.sport_or_category,
    serial_number: stripMarkdown(convInfo.serial_number) || card.serial_numbering || dvgGrading.card_info?.serial_number,
    rookie_or_first: convInfo.rookie_or_first || card.rookie_card || dvgGrading.card_info?.rookie_or_first,
    subset: subsetRaw,
    rarity_tier: stripMarkdown(convInfo.rarity_tier) || card.rarity_tier || card.rarity_description || dvgGrading.card_info?.rarity_tier,
    autographed: convInfo.autographed === true || card.autographed === true || card.autograph_type === 'authentic',
    memorabilia: convInfo.memorabilia === true || card.memorabilia_type !== 'none',
    card_front_text: convInfo.card_front_text || dvgGrading.card_info?.card_front_text,
    card_back_text: convInfo.card_back_text || dvgGrading.card_info?.card_back_text,
    // Pokemon-specific fields
    pokemon_type: stripMarkdown(convInfo.pokemon_type) || card.pokemon_type || null,
    pokemon_stage: stripMarkdown(convInfo.pokemon_stage) || card.pokemon_stage || null,
    hp: stripMarkdown(convInfo.hp) || card.hp || null,
    card_type: stripMarkdown(convInfo.card_type) || card.card_type || null,

    // 🏴‍☠️ ONE PIECE-SPECIFIC FIELDS - From conversational_card_info (Database data merged in by API)
    ygo_card_type: stripMarkdown(convInfo.ygo_card_type) || card.ygo_card_type || null,
    ygo_attribute: stripMarkdown(convInfo.card_color) || card.ygo_attribute || null,
    ygo_atk: convInfo.card_power ?? card.ygo_atk ?? null,
    ygo_def: convInfo.card_cost ?? card.ygo_def ?? null,
    ygo_level: convInfo.life ?? card.ygo_level ?? null,
    ygo_scale: convInfo.counter_amount ?? card.ygo_scale ?? null,
    ygo_race: stripMarkdown(convInfo.attribute) || card.ygo_race || null,
    ygo_archetype: stripMarkdown(convInfo.sub_types) || card.ygo_archetype || null,
    ygo_frame_type: stripMarkdown(convInfo.variant_type) || card.ygo_frame_type || null,
    expansion_code: stripMarkdown(convInfo.expansion_code) || card.expansion_code || null,
    collector_number: stripMarkdown(convInfo.collector_number) || stripMarkdown(convInfo.card_number) || card.card_number || null,
    artist_name: stripMarkdown(convInfo.artist_name) || card.artist_name || null,
    rarity_or_variant: stripMarkdown(convInfo.rarity_or_variant) || card.rarity_description || null,
    // Boolean fields
    is_foil: convInfo.is_foil ?? card.is_foil ?? false,
    is_promo: convInfo.is_promo ?? card.is_promo ?? false,
    // Yu-Gi-Oh variant features
    is_parallel: convInfo.is_parallel ?? (card.ygo_frame_type?.includes('parallel') ?? false),
    is_manga_art: convInfo.is_manga_art ?? (card.ygo_frame_type?.includes('manga') ?? false),
    is_alternate_art: convInfo.is_alternate_art ?? (card.ygo_frame_type === 'alternate_art'),
    is_sp: convInfo.is_sp ?? (card.ygo_frame_type === 'sp'),
    // Other fields
    ...
    // Market pricing (if available from merged API data)
  ...
  // 🎯 Yu-Gi-Oh cards use conversational grading as PRIMARY source
  ...
  const currentUrl = `${origin}/yugioh/${cardId}`;
  ...
  // 🎯 Yu-Gi-Oh cards: Map conversational data to expected structure
  ...
  // For Yu-Gi-Oh cards, create gradingScale from conversational data
  ...
  // 🎯 Yu-Gi-Oh cards: Extract centering from conversational_grading JSON as primary source
  ...
  if (card.conversational_grading) {
    try {
      ...
      if (parsed.centering) {
        ...
        console.log('[Yu-Gi-Oh Page] 🔧 Extracted centering from conversational_grading JSON:', parsedCenteringFromJSON);
      }
    } catch (e) {
      console.log('[Yu-Gi-Oh Page] Could not parse conversational_grading for centering:', e);
    }
  }

  // 🎯 Yu-Gi-Oh cards: Use parsed JSON first, then fall back to other sources
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl w-full">
            ...
            >
              ...
              <div
                ...
                onClick={() => openZoomModal(card.front_url, "Yu-Gi-Oh card front", "Card Front - Click for detailed view")}
              >
                <Image
                  src={card.front_url}
                  alt="Yu-Gi-Oh card front"
                  width={400}
                  height={560}
                  className="w-full"
                  priority
            ...
            >
              ...
              <div
                className="relative cursor-pointer transition-transform hover:scale-[1.02]"
                onClick={() => openZoomModal(card.back_url, "Yu-Gi-Oh card back", "Card Back - Click for detailed view")}
              >
                <Image
                  src={card.back_url}
            ...
            <div className="space-y-6">
              ...
              {(() => {
                ...
                return (
                  <div id="tour-download-buttons" className="flex flex-col md:flex-row items-center justify-between gap-4 my-6 px-4">
                    ...
                    {isOwner && <DownloadReportButton card={card} cardType={"yugioh" as any} showFounderEmblem={showFounderEmblem} showVipEmblem={showVipEmblem} showCardLoversEmblem={showCardLoversEmblem} labelStyle={labelStyle} customLabelConfig={activeConfig} />}
              ...
              <div id="tour-card-info" className="bg-white rounded-xl shadow-lg border-2 border-gray-200 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {(cardInfo.ygo_card_type || card.ygo_card_type) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Card Type</p>
                  ...
                  {/* Color */}
                  {(cardInfo.ygo_attribute || card.ygo_attribute) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Color</p>
                      <div className="flex gap-1 flex-wrap">
                        {(cardInfo.ygo_attribute || card.ygo_attribute)?.split('/').map((color: string, idx: number) => {
                          const colorMap: {[key: string]: {name: string, bg: string, text: string}} = {
                            'Red': {name: 'Red', bg: 'bg-red-500', text: 'text-white'},
                            'Blue': {name: 'Blue', bg: 'bg-blue-500', text: 'text-white'},
                            'Green': {name: 'Green', bg: 'bg-green-500', text: 'text-white'},
                            'Purple': {name: 'Purple', bg: 'bg-purple-500', text: 'text-white'},
                            'Black': {name: 'Black', bg: 'bg-gray-800', text: 'text-white'},
                            'Yellow': {name: 'Yellow', bg: 'bg-yellow-400', text: 'text-gray-900'}
                          };
                          const trimmedColor = color.trim();
                          const colorInfo = colorMap[trimmedColor] || {name: trimmedColor, bg: 'bg-gray-200', text: 'text-gray-700'};
                          return (
                            <span
                              key={idx}
                  ...
                  {/* Power (Characters and Leaders) */}
                  {(cardInfo.ygo_atk || card.ygo_atk) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Power</p>
                  ...
                  {/* Cost (DON!!) */}
                  {(cardInfo.ygo_def !== undefined && cardInfo.ygo_def !== null) || (card.ygo_def !== undefined && card.ygo_def !== null) ? (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Cost (DON!!)</p>
                      <p className="font-semibold text-blue-600 text-lg">
                        {cardInfo.ygo_def ?? card.ygo_def}
                      </p>
                    </div>
                  ) : null}

                  {/* Life (Leaders only) */}
                  {(cardInfo.ygo_level !== undefined && cardInfo.ygo_level !== null) || (card.ygo_level !== undefined && card.ygo_level !== null) ? (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Life</p>
                      <p className="font-semibold text-red-600 text-lg">
                        {cardInfo.ygo_level ?? card.ygo_level}
                      </p>
                    </div>
                  ) : null}

                  {/* Counter */}
                  {(cardInfo.ygo_scale !== undefined && cardInfo.ygo_scale !== null) || (card.ygo_scale !== undefined && card.ygo_scale !== null) ? (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Counter</p>
                      <p className="font-semibold text-green-600">
                        +{cardInfo.ygo_scale ?? card.ygo_scale}
                      </p>
                    </div>
                  ) : null}

                  {/* Attribute (Slash, Strike, Ranged, etc.) */}
                  {(cardInfo.ygo_race || card.ygo_race) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Attribute</p>
                      <p className="font-semibold text-gray-900">
                        {cardInfo.ygo_race || card.ygo_race}
                      </p>
                    </div>
                  ...
                  {/* Sub Types / Affiliations */}
                  {(cardInfo.ygo_archetype || card.ygo_archetype) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Affiliations</p>
                  ...
                  {/* Variant Type */}
                  {(cardInfo.ygo_frame_type || card.ygo_frame_type) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Variant</p>
                      <span className="px-2 py-1 rounded-lg text-xs font-bold bg-gradient-to-r from-purple-500 to-pink-500 text-white">
                        {(cardInfo.ygo_frame_type || card.ygo_frame_type)?.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                      </span>
                    </div>
                  )}
                  ...
                  {/* Expansion Code (if available) */}
                  {cardInfo.expansion_code && (
                    <div className="space-y-1">
                  ...
                  {/* Frame Version */}
                  {((cardInfo as any).frame_version || card.frame_version) && (
                    <div className="space-y-1">
                  ...
                  {((cardInfo as any).is_double_faced || card.is_double_faced) && !card.is_double_faced && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Card Layout</p>
                ...
                {(() => {
                  ...
                  // Check if we have any special features to show (Yu-Gi-Oh specific)
                  const hasSpecialFeatures = dvgGrading.rarity_features || cardInfo.serial_number ||
                    ...
                    cardInfo.ygo_frame_type || cardInfo.is_parallel || cardInfo.is_manga_art ||
                    cardInfo.is_alternate_art || cardInfo.is_sp || cardInfo.is_promo;
                  ...
                  <div className="border-t pt-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {/* Parallel Art - Yu-Gi-Oh specific */}
                      {(cardInfo.is_parallel || cardInfo.ygo_frame_type?.includes('parallel')) && (
                        <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-3 border border-purple-200">
                          <p className="text-purple-700 text-xs font-semibold mb-1">VARIANT</p>
                          <p className="font-bold text-purple-900">✨ Parallel Art</p>
                        </div>
                      )}

                      {/* Manga Art - Yu-Gi-Oh specific */}
                      {(cardInfo.is_manga_art || cardInfo.ygo_frame_type?.includes('manga')) && !cardInfo.ygo_frame_type?.includes('parallel_manga') && (
                        <div className="bg-orange-50 rounded-lg p-3 border border-orange-200">
                          <p className="text-orange-700 text-xs font-semibold mb-1">VARIANT</p>
                          <p className="font-bold text-orange-900">📖 Manga Art</p>
                        </div>
                      )}

                      {/* Parallel Manga - Yu-Gi-Oh specific (combines both) */}
                      {cardInfo.ygo_frame_type === 'parallel_manga' && (
                        <div className="bg-gradient-to-br from-purple-100 to-orange-100 rounded-lg p-3 border border-purple-300">
                          <p className="text-purple-700 text-xs font-semibold mb-1">VARIANT</p>
                          <p className="font-bold text-purple-900">✨📖 Parallel Manga</p>
                        </div>
                      )}

                      {/* SP (Special) Card - Yu-Gi-Oh specific */}
                      {(cardInfo.is_sp || cardInfo.ygo_frame_type === 'sp') && (
                        <div className="bg-yellow-50 rounded-lg p-3 border border-yellow-300">
                          <p className="text-yellow-700 text-xs font-semibold mb-1">SPECIAL</p>
                          <p className="font-bold text-yellow-800">🌟 SP Card</p>
                        </div>
                      )}

                      {/* Alternate Art - Yu-Gi-Oh specific */}
                      {(cardInfo.is_alternate_art || cardInfo.ygo_frame_type === 'alternate_art') && (
                        <div className="bg-indigo-50 rounded-lg p-3 border border-indigo-200">
                          <p className="text-indigo-700 text-xs font-semibold mb-1">VARIANT</p>
                          <p className="font-bold text-indigo-900">🎨 Alternate Art</p>
                        </div>
                      )}
                      ...
                      {cardInfo.is_promo && (
                        <div className="bg-cyan-50 rounded-lg p-3 border border-cyan-200">
                          <p className="text-cyan-700 text-xs font-semibold mb-1">PROMO</p>
                          <p className="font-bold text-cyan-900">🎁 Promo Card</p>
                        </div>
                      )}
                      {/* Foil - if applicable */}
                      {card.is_foil && (
                        <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-lg p-3 border border-purple-200">
                          ...
                          <p className="font-bold text-purple-900 capitalize">{card.foil_type || 'Foil'}</p>
                        </div>
                      )}
                    </div>
                ...
                {/* Yu-Gi-Oh Card Text Section - Abilities, Effects, Flavor Text */}
                  ...
                  return (
                    <OtherPriceLookup
                      ...
                      card={{
                        ...
                        card_number: (cardInfo as any).card_id || cardInfo.card_number || card.card_number,
                        year: (cardInfo as any).set_year || card.release_date,
                        manufacturer: 'Konami',
              ...
              <div id="tour-market-pricing" className="bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  {/* eBay General Search */}
                  <a
                    href={generateOnePieceEbaySearchUrl({
                  ...
                  {/* eBay Sold Listings */}
                  <a
                    href={generateOnePieceEbaySoldListingsUrl({
                      card_name: extractEnglishForSearch(cardInfo.card_name) || extractEnglishForSearch(card.card_name),
                      featured: extractEnglishForSearch(cardInfo.player_or_character) || extractEnglishForSearch(card.featured),
                      card_number: (cardInfo as any).card_id || cardInfo.card_number || card.card_number
                    } as CardData)}
                  ...
                  <a
                    href={dcmPriceData?.priceChartingUrl || `https://www.pricecharting.com/search-products?q=${encodeURIComponent(
                      [
                        ...
                        (cardInfo as any).card_id || cardInfo.card_number || card.card_number
                      ].filter(Boolean).join(' ')
                    )}&type=prices`}
                    target="_blank"
                    rel="noopener noreferrer"
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-gray-700">
                          ...
                          <ul className="list-disc list-inside space-y-0.5 text-gray-600">
                            ...
                            <li>Pre-filled Yu-Gi-Oh! TCG item specifics</li>
                            <li>Set name, card number, and rarity</li>
                          </ul>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-8">
                      {(() => {
                        ...
                        if (isJSONReport && jsonData) {
                          ...
                          // Card Information Section - Enhanced for Yu-Gi-Oh with Database API data
                          const renderCardInfo = () => {
                            // 🏴‍☠️ Yu-Gi-Oh: Use conversational_card_info (has merged Database data) as primary source
                            ...
                            const manufacturer = apiInfo.manufacturer || info.manufacturer || 'Konami';
                            const rarity = apiInfo.rarity_or_variant || apiInfo.rarity_tier || info.rarity_tier;

                            // Yu-Gi-Oh-specific fields from database
                            const ygoCardType = apiInfo.ygo_card_type || apiInfo.card_type;
                            const ygoAttribute = apiInfo.ygo_attribute || apiInfo.attribute;
                            const ygoAtk = apiInfo.ygo_atk ?? apiInfo.atk;
                            const ygoDef = apiInfo.ygo_def ?? apiInfo.def;
                            const ygoLevel = apiInfo.ygo_level ?? apiInfo.level;
                            const ygoScale = apiInfo.ygo_scale ?? apiInfo.scale;
                            const ygoRace = apiInfo.ygo_race || apiInfo.race;
                            const ygoArchetype = apiInfo.ygo_archetype || apiInfo.archetype;
                            const ygoFrameType = apiInfo.ygo_frame_type || apiInfo.frame_type;
                            const ygoLinkval = apiInfo.ygo_linkval ?? apiInfo.linkval;
                            ...
                            return (
                              <>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 mt-4">
                                  ...
                                  {ygoCardType && renderField('Card Type', ygoCardType)}
                                  {ygoAttribute && renderField('Attribute', ygoAttribute)}
                                  {ygoRace && renderField('Type', ygoRace)}
                                  {ygoAtk !== undefined && ygoAtk !== null && renderField('ATK', String(ygoAtk))}
                                  {ygoDef !== undefined && ygoDef !== null && renderField('DEF', String(ygoDef))}
                                  {ygoLevel !== undefined && ygoLevel !== null && renderField('Level/Rank', String(ygoLevel))}
                                  {ygoLinkval !== undefined && ygoLinkval !== null && renderField('Link Rating', String(ygoLinkval))}
                                  {ygoScale !== undefined && ygoScale !== null && renderField('Pendulum Scale', String(ygoScale))}
                                  {ygoArchetype && renderField('Archetype', ygoArchetype)}
                                  {ygoFrameType && renderField('Frame', ygoFrameType.replace(/_/g, ' '))}
        ...
        <EditCardLabelModal
          ...
          onSaved={async () => {
            try {
              const session = getStoredSession();
              const userParam = session?.user?.id ? `&user_id=${session.user.id}` : '';
              const res = await fetch(`/api/yugioh/${card.id}?t=${Date.now()}${userParam}`);
              if (res.ok) {
                const data = await res.json();
                setCard(data);
              }
