...
import type { PopCardRow, PopPagination } from '@/lib/pop/popData';
...
/**
 * The interactive half of the population table.
 *
 * It is a client component, but it is handed the first page of rows by the
 * server, so the table is fully present in the initial HTML. It only calls the
 * API once the reader changes the search, the page, or the page size.
 */
export default function PopCategoryTable({
  category,
  initialCards,
  initialPagination,
}: {
  category: CategoryInfo;
  initialCards: PopCardRow[];
  initialPagination: PopPagination;
}) {
  const isTcg = isTcgCategory(category.slug);
  const [cards, setCards] = useState<PopCardRow[]>(initialCards);
  const [pagination, setPagination] = useState<PopPagination>(initialPagination);
  ...
  const fetchCards = useCallback(
    (offset: number) => {
      ...
      fetch(`/api/pop/${category.slug}/sets?${qp}`)
        .then((res) => res.json())
        .then((data) => {
          setCards(data.cards || []);
          setPagination(
            data.pagination || { total: 0, limit: itemsPerPage, offset, hasMore: false }
          );
        })
        .catch((err) => console.error('Failed to load cards:', err))
        .finally(() => setLoading(false));
    },
    [category.slug, debouncedSearch, itemsPerPage]
  );

  // The server already rendered page one with the default page size; only go
  // back to the API once the reader actually changes something.
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <caption className="sr-only">
              {category.displayName} cards graded by DCM, with the count at each grade from 1 to 10
            </caption>
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                {GRADE_COLUMNS.map((g) => (
                  <th
                    key={g}
                    className={`px-3 py-3 font-semibold text-center min-w-[48px] ${gradeColor(g)}`}
                  >
                    {g}
                  </th>
                ))}
                ...
                Array.from({ length: 10 }).map((_, i) => (
                  <tr key={i} className="border-b border-gray-100">
                    ...
                    <td className="px-3 py-3 text-center">
                      <div className="h-4 w-8 bg-gray-100 rounded animate-pulse mx-auto" />
                    </td>
                    {GRADE_COLUMNS.map((g) => (
                ...
                <tr>
                  <td colSpan={colCount} className="px-4 py-12 text-center text-gray-500">
                    {debouncedSearch
                      ...
                      : 'No graded cards found for this category.'}
                  </td>
                </tr>
              ) : (
                sortedCards.map((card, idx) => (
                  <tr
                    key={`${card.cardName}-${card.cardNumber}-${idx}`}
                  ...
                  >
                    ...
                    {!isTcg && (
                      <td className="px-3 py-3 text-gray-600">{card.featured || '—'}</td>
                    )}
                    <td
                      className="px-3 py-3 text-gray-600 max-w-[200px] truncate"
                      title={card.cardSet || undefined}
                    >
                      {card.cardSet || '—'}
                    </td>
