...
export function SportsCardDetails() {
  ...
  const { balance, deductLocalCredit, refreshCredits, isFirstPurchase, isLoading: creditsLoading } = useCredits();
  ...
  const classicQrDataUrl = useClassicQrDataUrl(
    isClassicLabel && origin && cardId ? `${origin}/sports/${cardId}` : null
  );
  ...
  useEffect(() => {
    if (card && card.grade && !hasTrackedGradeComplete.current && typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'grade_card_complete', {
        card_category: card.category || 'Sports',
        grade: card.grade
      });
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-6">
          ...
          {dvgGrading && Object.keys(dvgGrading).length > 0 && (
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-lg p-6 border-2 border-gray-200">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-6">
                  <div className="space-y-1">
                    <p className="text-gray-500 text-xs uppercase tracking-wide">Player/Character</p>
                    <p className="font-bold text-gray-900 text-lg">{cardInfo.player_or_character || 'Unknown'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-500 text-xs uppercase tracking-wide">Set Name</p>
                    <p className="font-semibold text-gray-900">{cardInfo.set_name || 'Unknown'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-500 text-xs uppercase tracking-wide">Manufacturer</p>
                    <p className="font-semibold text-gray-900">{cardInfo.manufacturer || 'Unknown'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-500 text-xs uppercase tracking-wide">Year</p>
                    <p className="font-semibold text-gray-900">{cardInfo.year || 'Unknown'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-500 text-xs uppercase tracking-wide">Card Number</p>
                    <p className="font-semibold text-gray-900">{cardInfo.card_number || 'N/A'}</p>
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="border-t pt-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      {(cardInfo.memorabilia || dvgGrading.rarity_features?.memorabilia?.present) && (
                        <div className="bg-green-50 rounded-lg p-3 border border-green-200">
                          <p className="text-green-700 text-xs font-semibold mb-1">MEMORABILIA</p>
                          <p className="font-bold text-green-900">
                            🎽 {dvgGrading.rarity_features?.memorabilia?.type || 'Yes'}
                          </p>
                        </div>
                      )}
              ...
              {(() => {
                ...
                const frontCorners = detailsJson.front_corners || {};
                const backCorners = detailsJson.back_corners || {};
                const frontEdges = detailsJson.front_edges || {};
                const backEdges = detailsJson.back_edges || {};
                const frontSurface = detailsJson.front_surface || {};
                const backSurface = detailsJson.back_surface || {};
              ...
              <div id="tour-market-pricing" className="bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  ...
                  <a
                    href={generateEbaySearchUrl({
                      ...
                      manufacturer_name: cardInfo.manufacturer || card.manufacturer_name,
                      card_number: cardInfo.card_number || card.card_number,
                      subset: cardInfo.subset || card.subset,
                ...
                <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                  <p className="text-xs text-gray-600">
                    <strong>Note:</strong> eBay links will search for "{cardInfo.player_or_character || card.featured || card.card_name} {cardInfo.set_name || card.card_set}" and similar terms based on the detected card information.
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-lg border-2 border-amber-200 shadow-md hover:shadow-lg transition-shadow">
                      <div className="bg-gradient-to-r from-gray-900 to-gray-800 text-white px-4 py-3 rounded-t-lg">
                        <h3 className="text-lg font-bold">BGS</h3>
          ...
          {(() => {
            ...
            const href = outOfCredits ? '/credits' : '/upload?category=Sports';
        ...
        <EditCardLabelModal
          ...
          onSaved={async () => {
            try {
              const session = getStoredSession();
              const userParam = session?.user?.id ? `&user_id=${session.user.id}` : '';
              const res = await fetch(`/api/sports/${card.id}?t=${Date.now()}${userParam}`);
              if (res.ok) {
                const data = await res.json();
                setCard(data);
              }
