...
export async function GET(request: NextRequest, { params }: StarWarsCardGradingRequest) {
  ...
  try {
    ...
    if (!gradingLock.acquired) {
      console.log(`[GET /api/starwars/${cardId}] Another instance holds the grading lock, returning 429`);
      return NextResponse.json(
    ...
    if (frontUrl && backUrl) {
      try {
        ...
        const conversationalResult = await gradeCardConversational(frontUrl, backUrl, 'starwars' as any, {
    ...
    const idGuard = await preserveIdentityOnRegrade(supabase, cardId, updateData as any, { forceRegrade, reidentify, tag: `GET /api/starwars/${cardId}` });
