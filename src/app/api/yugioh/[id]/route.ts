...
// Track Yu-Gi-Oh cards currently being processed with timestamps
const processingYugiohCards = new Map<string, number>();
...
const cleanupStuckCards = () => {
  ...
  for (const [cardId, timestamp] of processingYugiohCards.entries()) {
    if (timestamp < twoMinutesAgo) {
      ...
      processingYugiohCards.delete(cardId);
    }
  }
};

// Types
type YugiohCardGradingRequest = {
...
// Extract Yu-Gi-Oh card grade information
function extractYugiohGradeInfo(gradingResult: any) {
...
// Extract key fields for database columns (Yu-Gi-Oh-specific) - Legacy Assistant API
function extractYugiohCardFields(gradingResult: any) {
  ...
  return {
    ...
    manufacturer_name: cardInfo["Manufacturer Name"] || 'Konami',
    release_date: cardInfo["Release Date"] || null,
    authentic: cardInfo["Authentic"] || null,
    // Yu-Gi-Oh-specific fields
    ygo_card_type: cardDetails["Card Type"] || null,
    ygo_frame_type: cardDetails["Frame Type"] || null,
    ygo_attribute: cardDetails["Attribute"] || null,
    ygo_race: cardDetails["Race"] || cardDetails["Monster Type"] || null,
    ygo_atk: cardDetails["ATK"] || cardDetails["Attack"] || null,
    ygo_def: cardDetails["DEF"] || cardDetails["Defense"] || null,
    ygo_level: cardDetails["Level"] || cardDetails["Rank"] || null,
    ygo_archetype: cardDetails["Archetype"] || null,
    ygo_scale: cardDetails["Pendulum Scale"] || cardDetails["Scale"] || null,
    ygo_linkval: cardDetails["Link Rating"] || cardDetails["Link Value"] || null,
...
// Extract Yu-Gi-Oh-specific fields from conversational v4.2 JSON
function extractYugiohFieldsFromConversational(conversationalJSON: any) {
  try {
    ...
    return {
      ...
      manufacturer_name: cardInfo.manufacturer || 'Konami',
      ...
      // Yu-Gi-Oh-specific fields
      ygo_card_type: cardInfo.ygo_card_type || cardInfo.card_type || null,
      ygo_frame_type: cardInfo.frame_type || null,
      ygo_attribute: cardInfo.attribute || null,
      ygo_race: cardInfo.race || cardInfo.monster_type || null,
      ygo_atk: cardInfo.atk || cardInfo.attack || null,
      ygo_def: cardInfo.def || cardInfo.defense || null,
      ygo_level: cardInfo.level || cardInfo.rank || null,
      ygo_archetype: cardInfo.archetype || null,
      ygo_scale: cardInfo.scale || cardInfo.pendulum_scale || null,
      ygo_linkval: cardInfo.linkval || cardInfo.link_rating || null,
      is_foil: cardInfo.is_foil !== undefined ? cardInfo.is_foil : false
    };
  } catch (error) {
    console.error('[Yu-Gi-Oh Field Extraction] Error parsing conversational JSON:', error);
    return {};
  }
}
...
export async function GET(request: NextRequest, { params }: YugiohCardGradingRequest) {
  ...
  if (statusOnly) {
    console.log(`[GET /api/yugioh/${cardId}] Status-only check requested`);
    ...
    if (cardError || !card) {
      console.log(`[GET /api/yugioh/${cardId}] Status-only 404: error=${cardError?.message || 'none'}, code=${cardError?.code || 'none'}`);
      return NextResponse.json({ error: "Card not found" }, { status: 404 });
    }

    // Verify it's a Yu-Gi-Oh card
    if (card.category !== 'Yu-Gi-Oh') {
      return NextResponse.json({ error: "Not a Yu-Gi-Oh card" }, { status: 404 });
    }
    ...
    const isProcessing = processingYugiohCards.has(cardId);

    console.log(`[GET /api/yugioh/${cardId}] Status-only result: complete=${hasCompleteGrading}, processing=${isProcessing}`);
  ...
  // Check if already processing this Yu-Gi-Oh card
  if (processingYugiohCards.has(cardId)) {
    console.log(`[GET /api/yugioh/${cardId}] Yu-Gi-Oh card already being processed, returning 429`);
    return NextResponse.json(
      { error: "Yu-Gi-Oh card is being processed by another request. Please wait and refresh." },
      { status: 429 }
    );
  }

  try {
    processingYugiohCards.set(cardId, Date.now());
    const supabase = supabaseServer();

    // Get Yu-Gi-Oh card from database
    const { data: card, error: cardError } = await supabase
      .from("cards")
      .select("*")
      .eq("id", cardId)
      .eq("category", "Yu-Gi-Oh") // Ensure it's a Yu-Gi-Oh card
      .single();

    if (cardError || !card) {
      console.error(`[GET /api/yugioh/${cardId}] Card lookup error:`, cardError);
      console.log(`[GET /api/yugioh/${cardId}] Card data:`, card);
      return NextResponse.json({ error: "Yu-Gi-Oh card not found" }, { status: 404 });
    }

    console.log(`[GET /api/yugioh/${cardId}] Yu-Gi-Oh card found`);
    ...
    if (cardVisibility === 'private') {
      ...
      if (!userId) {
        console.log(`[GET /api/yugioh/${cardId}] Private card access denied - not authenticated`);
        return NextResponse.json({
      ...
      if (card.user_id !== userId) {
        console.log(`[GET /api/yugioh/${cardId}] Private card access denied - user ${userId} is not owner ${card.user_id}`);
        return NextResponse.json({
          error: "This card is private",
      ...
      console.log(`[GET /api/yugioh/${cardId}] Private card access granted to owner ${userId}`);
    } else {
      // Public card - anyone can view
      console.log(`[GET /api/yugioh/${cardId}] Public card - access granted`);
    }
    ...
    if (card.user_id) {
      try {
        ...
        console.error(`[GET /api/yugioh/${cardId}] Error fetching owner founder status:`, err);
      }
    }

    // Create signed URLs for Yu-Gi-Oh card images (parallel for speed)
    console.log(`[GET /api/yugioh/${cardId}] Creating signed URLs for front: ${card.front_path}, back: ${card.back_path}`);
    ...
    if (!frontUrl || !backUrl) {
      console.error(`[GET /api/yugioh/${cardId}] Failed to create signed URLs. Front: ${frontUrl}, Back: ${backUrl}`);
      return NextResponse.json({ error: "Failed to access Yu-Gi-Oh card images" }, { status: 500 });
    }
    console.log(`[GET /api/yugioh/${cardId}] Signed URLs created successfully`);

    // Check if Yu-Gi-Oh card already has complete grading data
    const hasCompleteGrading =
    ...
    if (hasCompleteGrading && !forceRegrade) {
      console.log(`[GET /api/yugioh/${cardId}] Yu-Gi-Oh card already fully processed, returning existing result`);

      // Parse conversational_grading if it exists
      let parsedConversationalData = null;
      if (card.conversational_grading) {
        try {
          console.log('[YU-GI-OH CACHE] Parsing cached conversational_grading JSON...');
          ...
          if (!parsedConversationalData.professional_grade_estimates && parsedConversationalData.decimal_grade && parsedConversationalData.decimal_grade !== 'N/A') {
            try {
              console.log('[YU-GI-OH CACHE] Calculating professional grade estimates...');
              ...
              console.log(`[YU-GI-OH CACHE] Professional estimates: PSA ${professionalEstimates.PSA.estimated_grade}, BGS ${professionalEstimates.BGS.estimated_grade}`);
            } catch (mapperError) {
              console.error('[YU-GI-OH CACHE] Professional mapper failed:', mapperError);
            }
          }

          console.log('[YU-GI-OH CACHE] Parsed cached JSON successfully');
        } catch (error) {
          console.error('[YU-GI-OH CACHE] Failed to parse cached JSON:', error);
        }
      }
    ...
    if (!hasCompleteGrading) {
      console.log(`[GET /api/yugioh/${cardId}] Yu-Gi-Oh card needs grading analysis`);
    } else if (forceRegrade) {
      console.log(`[GET /api/yugioh/${cardId}] Force re-grade requested, bypassing cache`);
    }
    ...
    if (!gradingLock.acquired) {
      console.log(`[GET /api/yugioh/${cardId}] Another instance holds the grading lock, returning 429`);
      return NextResponse.json(
    ...
    console.log(`[GET /api/yugioh/${cardId}] Starting Yu-Gi-Oh card conversational AI grading (v4.2)...`);
    ...
    if (frontUrl && backUrl) {
      try {
        console.log(`[YU-GI-OH CONVERSATIONAL AI v4.2] Starting PRIMARY grading with Yu-Gi-Oh-specific prompt...`);
        ...
        console.log(`[YU-GI-OH CONVERSATIONAL AI] Output format detected: ${isJSONMode ? 'JSON' : 'Markdown'} (${conversationalResult.meta?.version || 'unknown'})`);

        console.log(`[GET /api/yugioh/${cardId}] Conversational grading completed: ${conversationalResult.extracted_grade?.decimal_grade || 'N/A'}`);
        ...
        if (conversationalGradingResult) {
          try {
            ...
            console.error(`[GET /api/yugioh/${cardId}] Failed to create legacy gradingResult:`, e);
          }
        }
      } catch (error: any) {
    ...
    if (conversationalGradingResult) {
      try {
        console.log(`[GET /api/yugioh/${cardId}] Parsing conversational JSON...`);
        const jsonData = JSON.parse(conversationalGradingResult);

        console.log(`[GET /api/yugioh/${cardId}] DEBUG Raw AI card_info:`, JSON.stringify(jsonData.card_info, null, 2));
        ...
        console.log(`[GET /api/yugioh/${cardId}] Parsed conversational data:`, {
        ...
        if (conversationalGradingData.decimal_grade && conversationalGradingData.decimal_grade !== 'N/A') {
          try {
            console.log(`[GET /api/yugioh/${cardId}] Calling professional grade mapper (deterministic)...`);
            ...
            console.log(`[GET /api/yugioh/${cardId}] Professional estimates: PSA ${professionalEstimates.PSA.estimated_grade}, BGS ${professionalEstimates.BGS.estimated_grade}, SGC ${professionalEstimates.SGC.estimated_grade}, CGC ${professionalEstimates.CGC.estimated_grade}`);
          } catch (mapperError) {
            console.error(`[GET /api/yugioh/${cardId}] Professional mapper failed:`, mapperError);
            conversationalGradingData.professional_grade_estimates = null;
          }
        } else {
          console.log(`[GET /api/yugioh/${cardId}] Skipping professional mapper (grade is N/A or null)`);
          conversationalGradingData.professional_grade_estimates = null;
        }

      } catch (error) {
        console.error(`[GET /api/yugioh/${cardId}] Failed to parse conversational JSON:`, error);
      }
    }
    ...
    if (conversationalGradingData?.card_info) {
      try {
        ...
        const matchResult = await lookupYugiohCard(
        ...
        if (trustworthy && candidate) {
          ...
          conversationalGradingData.card_info = {
            ...
            card_number: dbCard.set_code,
            set_name: dbCard.set_name,
            // === YU-GI-OH-SPECIFIC ATTRIBUTES (from database) ===
            ygo_card_type: dbCard.type,
            frame_type: dbCard.frame_type,
            ...
            level: dbCard.level,
            scale: dbCard.scale,
            linkval: dbCard.linkval,
            linkmarkers: dbCard.linkmarkers,
            card_desc: dbCard.card_desc,
            // === RARITY & VARIANT (from database) ===
            rarity: dbCard.set_rarity,
            rarity_or_variant: dbCard.set_rarity,
            // === PRICING (from database if available) ===
            market_price: dbCard.set_price || dbCard.tcgplayer_price || dbCard.cardmarket_price,
            tcgplayer_price: dbCard.tcgplayer_price,
            cardmarket_price: dbCard.cardmarket_price,
            // === DATABASE REFERENCE ===
            _database_match: {
              yugioh_card_id: dbCard.card_id,
          ...
          console.log(`[GET /api/yugioh/${cardId}] Card info enhanced with database data`);
        } else {
          console.log(`[GET /api/yugioh/${cardId}] No high-confidence database match found`);
        }
      } catch (dbLookupError) {
        console.error(`[GET /api/yugioh/${cardId}] Database lookup failed:`, dbLookupError);
      }
    }

    // Extract Yu-Gi-Oh card grade information
    const { rawGrade, wholeGrade, confidence } = extractYugiohGradeInfo(gradingResult);

    // Extract key fields for database columns (Yu-Gi-Oh-specific)
    const cardFieldsLegacy = extractYugiohCardFields(gradingResult);

    // Extract Yu-Gi-Oh-specific fields from conversational v4.2 JSON
    const cardFieldsConversational = conversationalGradingData?.card_info
      ? extractYugiohFieldsFromConversational(conversationalGradingData)
      : {};
    ...
    // Update database with comprehensive Yu-Gi-Oh card data
    const updateData = {
      ...
      // Individual searchable/sortable columns (Yu-Gi-Oh-specific - merged from both sources)
      ...cardFields,

      // Database-matched card reference
      ...(matchedDatabaseCard && {
        yugioh_card_id: matchedDatabaseCard.card_id,
        yugioh_reference_image: matchedDatabaseCard.image_url,
        yugioh_database_match_confidence: databaseMatchConfidence
      }),
    ...
    console.log(`[GET /api/yugioh/${cardId}] Updating database with extracted Yu-Gi-Oh fields:`, {
      card_name: cardFields.card_name,
      card_set: cardFields.card_set,
      ygo_card_type: cardFields.ygo_card_type,
      ygo_attribute: cardFields.ygo_attribute,
      ygo_archetype: cardFields.ygo_archetype,
      grade: wholeGrade
    });
    ...
    const idGuard = await preserveIdentityOnRegrade(supabase, cardId, updateData as any, { forceRegrade, reidentify, tag: `GET /api/yugioh/${cardId}` });
    ...
    // If update fails due to missing columns (migration not applied), retry without Yu-Gi-Oh specific columns
    if (updateError?.code === 'PGRST204') {
      console.warn(`[GET /api/yugioh/${cardId}] Some columns not found, retrying without Yu-Gi-Oh specific columns. Run migration: migrations/add_yugioh_card_reference_columns.sql`);

      // Remove Yu-Gi-Oh specific columns that may not exist
      const fallbackUpdateData = { ...updateData };
      delete (fallbackUpdateData as any).yugioh_card_id;
      delete (fallbackUpdateData as any).yugioh_reference_image;
      delete (fallbackUpdateData as any).yugioh_database_match_confidence;
      delete (fallbackUpdateData as any).ygo_card_type;
      delete (fallbackUpdateData as any).ygo_frame_type;
      delete (fallbackUpdateData as any).ygo_attribute;
      delete (fallbackUpdateData as any).ygo_race;
      delete (fallbackUpdateData as any).ygo_atk;
      delete (fallbackUpdateData as any).ygo_def;
      delete (fallbackUpdateData as any).ygo_level;
      delete (fallbackUpdateData as any).ygo_archetype;
      delete (fallbackUpdateData as any).ygo_scale;
      delete (fallbackUpdateData as any).ygo_linkval;
    ...
    return NextResponse.json({
      ...
      back_url: backUrl,
      processing_time: Date.now() - startTime,
      yugioh_card_id: matchedDatabaseCard?.card_id || null,
      yugioh_reference_image: matchedDatabaseCard?.image_url || null,
      yugioh_database_match_confidence: databaseMatchConfidence,
    ...
    console.error(`[GET /api/yugioh/${cardId}] Error:`, error.message);
    ...
    return NextResponse.json(
      { error: "Failed to process Yu-Gi-Oh card: " + error.message },
      { status: 500 }
    );
  } finally {
    processingYugiohCards.delete(cardId);
    console.log(`[GET /api/yugioh/${cardId}] Removed from processing set`);
  }
}
...
export async function PATCH(request: NextRequest, { params }: YugiohCardGradingRequest) {
  ...
  try {
    ...
    console.log(`[PATCH /api/yugioh/${cardId}] Update data:`, body);

    // Update the Yu-Gi-Oh card
    ...
    if (error) {
      console.error(`[PATCH /api/yugioh/${cardId}] Update failed:`, error);
      return NextResponse.json({ error: "Failed to update Yu-Gi-Oh card" }, { status: 500 });
    }

    console.log(`[PATCH /api/yugioh/${cardId}] Yu-Gi-Oh card updated successfully`);
    return NextResponse.json(data);

  } catch (error: any) {
    console.error(`[PATCH /api/yugioh/${cardId}] Error:`, error.message);
    return NextResponse.json(
      { error: "Failed to update Yu-Gi-Oh card: " + error.message },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest, { params }: YugiohCardGradingRequest) {
  ...
  try {
    ...
    // Get the Yu-Gi-Oh card and verify ownership
    ...
    if (fetchError || !card) {
      console.error(`[DELETE /api/yugioh/${cardId}] Yu-Gi-Oh card not found:`, fetchError);
      return NextResponse.json({ error: "Yu-Gi-Oh card not found" }, { status: 404 });
    }
    ...
    if (deleteError) {
      console.error(`[DELETE /api/yugioh/${cardId}] Deletion failed:`, deleteError);
      return NextResponse.json({ error: "Failed to delete Yu-Gi-Oh card" }, { status: 500 });
    }

    console.log(`[DELETE /api/yugioh/${cardId}] Yu-Gi-Oh card deleted successfully`);
    return NextResponse.json({ success: true, message: "Yu-Gi-Oh card deleted successfully" });

  } catch (error: any) {
    console.error(`[DELETE /api/yugioh/${cardId}] Error:`, error.message);
    return NextResponse.json(
      { error: "Failed to delete Yu-Gi-Oh card: " + error.message },
      { status: 500 }
    );
  }
}
