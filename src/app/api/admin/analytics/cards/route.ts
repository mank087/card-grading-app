 ...
 *
 * Thin wrapper over get_card_analytics(). See
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('get_card_analytics', {
      p_from: from.toISOString(),
      p_to: to.toISOString(),
    })
    if (error) {
      console.error('[admin/analytics/cards] RPC error:', error)
