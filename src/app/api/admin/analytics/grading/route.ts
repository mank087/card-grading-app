 ...
 *
 * Thin wrapper over get_grading_analytics(). See
 * migrations/add_admin_analytics_rpcs.sql for the SQL aggregation logic.
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('get_grading_analytics', {
      p_from: from.toISOString(),
      p_to: to.toISOString(),
    })
    if (error) {
      console.error('[admin/analytics/grading] RPC error:', error)
