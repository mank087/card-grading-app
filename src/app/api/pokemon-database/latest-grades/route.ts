...
/**
 * GET /api/pokemon-database/latest-grades
 *
 * Fetches the latest 10 Pokemon cards graded through DCM
 * Returns public cards with signed image URLs
 */
export async function GET() {
  try {
    // Fetch latest graded Pokemon cards
    ...
    if (error) {
      console.error('[Latest Pokemon Grades] Database error:', error)
      throw error
    }
    ...
    if (signError) {
      console.error('[Latest Pokemon Grades] Signed URL error:', signError)
    ...
    console.error('[Latest Pokemon Grades] Error:', error)
    return NextResponse.json(
