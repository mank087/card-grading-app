  ...
  try {
    ...
    if (frontUrl && backUrl) {
      try {
        ...
        const failure = await recordGradingFailure({ chargeId: forceRegrade ? null : undefined, cardId, userId: card.user_id, category: 'One Piece', errorMessage: error.message });
        return NextResponse.json({
    ...
    const idGuard = await preserveIdentityOnRegrade(supabase, cardId, updateData as any, { forceRegrade, reidentify, tag: `GET /api/onepiece/${cardId}` });
    ...
    keepAliveAfterResponse(linkFromFirstLook(supabase, cardId, { afterGradeSave: true })
      .then(r => { if (r.status === 'linked') console.log(`[GET /api/onepiece/${cardId}] catalog linked from first look: ${r.name}`); }));
