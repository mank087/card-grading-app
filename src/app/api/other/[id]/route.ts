...
export async function GET(request: NextRequest, { params }: OtherCardGradingRequest) {
  ...
  try {
    ...
    if (!gradingLock.acquired) {
      console.log(`[GET /api/other/${cardId}] Another instance holds the grading lock, returning 429`);
      return NextResponse.json(
    ...
    const idGuard = await preserveIdentityOnRegrade(supabase, cardId, updateData, { forceRegrade, reidentify, tag: `GET /api/other/${cardId}` });
    ...
    if (updateError) {
      ...
      // Previously this logged and FELL THROUGH, returning 200 with a grade the
