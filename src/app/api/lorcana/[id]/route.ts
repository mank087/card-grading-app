...
export async function GET(request: NextRequest, { params }: LorcanaCardGradingRequest) {
  ...
  try {
    ...
    if (!gradingLock.acquired) {
      console.log(`[GET /api/lorcana/${cardId}] Another instance holds the grading lock, returning 429`);
      return NextResponse.json(
    ...
    if (frontUrl && backUrl) {
      try {
        ...
        const failure = await recordGradingFailure({ chargeId: forceRegrade ? null : undefined, cardId, userId: card.user_id, category: 'Lorcana', errorMessage: error.message });
        return NextResponse.json({
    ...
    const idGuard = await preserveIdentityOnRegrade(supabase, cardId, updateData as any, { forceRegrade, reidentify, tag: `GET /api/lorcana/${cardId}` });
    ...
    if (updateError) {
      ...
      return NextResponse.json({
        error: "Failed to save Lorcana card grading results",
        grading_failed: true,
        credit_refunded: failure.refunded, credit_refund_status: failure.refundStatus
      }, { status: 500 });
    }
    ...
    keepAliveAfterResponse(linkFromFirstLook(supabase, cardId, { afterGradeSave: true })
      .then(r => { if (r.status === 'linked') console.log(`[GET /api/lorcana/${cardId}] catalog linked from first look: ${r.name}`); }));
