...
export async function GET(request: NextRequest, { params }: LorcanaCardGradingRequest) {
  ...
  console.log(`[GET /api/lorcana/${cardId}] Starting Lorcana card grading request (force_regrade: ${forceRegrade})`);
  ...
  try {
    ...
    if (cardVisibility === 'private') {
      ...
      if (card.user_id !== user.id) {
        console.log(`[GET /api/lorcana/${cardId}] 🔒 Private card access denied - user ${user.id} is not owner ${card.user_id}`);
        return NextResponse.json({
          error: "This card is private",
      ...
      console.log(`[GET /api/lorcana/${cardId}] ✅ Private card access granted to owner ${user.id}`);
    } else {
      // Public card - anyone can view
      console.log(`[GET /api/lorcana/${cardId}] 🌐 Public card - access granted`);
    }

    // Create signed URLs for Lorcana card images
...
export async function DELETE(request: NextRequest, { params }: LorcanaCardGradingRequest) {
  ...
  try {
    ...
    // Delete the Lorcana card and its associated images
