...
export async function GET(request: NextRequest, { params }: PokemonCardGradingRequest) {
  ...
  console.log(`[GET /api/pokemon/${cardId}] Starting Pokemon card grading request (force_regrade: ${forceRegrade})`);
  ...
  try {
    ...
    if (cardVisibility === 'private') {
      ...
      if (card.user_id !== user.id) {
        console.log(`[GET /api/pokemon/${cardId}] 🔒 Private card access denied - user ${user.id} is not owner ${card.user_id}`);
        return NextResponse.json({
          error: "This card is private",
      ...
      console.log(`[GET /api/pokemon/${cardId}] ✅ Private card access granted to owner ${user.id}`);
    } else {
...
export async function DELETE(request: NextRequest, { params }: PokemonCardGradingRequest) {
  ...
  try {
    ...
    // Delete the Pokemon card and its associated images
