...
export async function GET(request: NextRequest, { params }: PokemonCardGradingRequest) {
  ...
  try {
    ...
    if (!gradingLock.acquired) {
      console.log(`[GET /api/pokemon/${cardId}] Another instance holds the grading lock, returning 429`);
      return NextResponse.json(
    ...
    if (frontUrl && backUrl) {
      try {
        ...
        console.log(`[POKEMON CONVERSATIONAL AI] Output format detected: ${isJSONMode ? 'JSON' : 'Markdown'} (${conversationalResult.meta?.version || 'unknown'})`);
