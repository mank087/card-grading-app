...
interface SportsAIGrading {
  ...
  // (second "Image Conditions" declaration merged into the one above)
  ...
  // Top-level fields present on some grading payload versions
  "Centering_Measurements"?: any;
  centerings_used?: any;
  category_scores?: Record<string, {score: number; weight: number; contribution?: number}>;
  front_specific_feedback?: any;
  back_specific_feedback?: any;
  text_transcription_summary?: any;
}
...
interface SportsCard {
  ...
  // 🏴‍☠️ One Piece database verification
  onepiece_card_id?: string | null;
  onepiece_reference_image?: string | null;
  onepiece_database_match_confidence?: 'high' | 'medium' | 'low' | null;

  // One Piece-specific fields
  op_card_type?: string | null;     // Leader, Character, Event, Stage
  op_card_color?: string | null;    // Red, Blue, Green, Purple, Black, Yellow (or multi-color)
  op_card_power?: number | null;    // Power value (Characters and Leaders)
  op_card_cost?: number | null;     // DON!! cost to play
  op_life?: number | null;          // Life value (Leaders only)
  op_counter?: number | null;       // Counter value: 1000, 2000, or NULL
  op_attribute?: string | null;     // Combat attribute: Slash, Strike, Ranged, Special, Wisdom
  op_sub_types?: string | null;     // Character traits/affiliations
  op_variant_type?: string | null;  // parallel, manga, parallel_manga, sp, alternate_art, etc.
  ...
  // Conversational grading structured payloads (deeply dynamic JSON)
...
function generateStructuredData(card: any, dvgGrading: any, cardUrl: string) {
  ...
  const productSchema = {
    ...
    category: 'One Piece TCG Trading Cards',
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: 'https://dcmgrading.com/upload/onepiece'
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: fullCardName || 'One Piece Card',
        item: cardUrl
      }
    ]
  };
...
export function OnePieceCardDetails() {
  ...
  const classicQrDataUrl = useClassicQrDataUrl(
    isClassicLabel && origin && cardId ? `${origin}/onepiece/${cardId}` : null
  );
  ...
  const fetchOnePieceCardDetails = useCallback(async () => {
    ...
    try {
      ...
      console.log(`[FRONTEND DEBUG] Calling One Piece API endpoint: /api/onepiece/${cardId}`);
      const res = await fetch(`/api/onepiece/${cardId}?t=${Date.now()}${userParam}`); // Cache-busting
      console.log(`[FRONTEND DEBUG] One Piece API response status: ${res.status}`);
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) {
          ...
          const retryWithBackoff = async (attempt: number = 1): Promise<void> => {
            ...
            try {
              console.log(`[FRONTEND DEBUG] Retry ${attempt}: Calling /api/onepiece/${cardId}`);
              const retryRes = await fetch(`/api/onepiece/${cardId}?t=${Date.now()}${userParam}`); // Cache-busting
  ...
  useEffect(() => {
    if (cardId) {
      console.log('Fetching One Piece Card Details for:', cardId);
      fetchOnePieceCardDetails();
    }
  }, [cardId, fetchOnePieceCardDetails]);
  ...
  useEffect(() => {
    if (card && card.grade && !hasTrackedGradeComplete.current && typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'grade_card_complete', {
        card_category: card.category || 'One Piece',
        grade: card.grade
      });
  ...
  const regradeCard = async () => {
    ...
    try {
      ...
      queueId = addToQueue({
        cardId: cardId,
        category: 'onepiece',
        categoryLabel: 'One Piece Card (Re-grade)',
        frontImageUrl: card.front_url,
        status: 'processing',
        resultUrl: `/onepiece/${cardId}`
      });
      console.log('[REGRADE] Added to queue:', queueId);

      // Call One Piece API with force_regrade parameter to bypass cache
      console.log('[REGRADE] Forcing fresh grading for One Piece card:', cardId);

      // Re-grading can take 30-60 seconds, so use a longer timeout
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 120000); // 120 second timeout

      const res = await fetch(`/api/onepiece/${cardId}?force_regrade=true&t=${Date.now()}`, {
    ...
    return (
      <div className="text-center p-8">
        ...
        <div className="flex gap-4 justify-center">
          ...
          <Link
            href="/upload?category=One Piece"
  ...
  if (!card) {
    return (
      <div className="text-center p-8">
        <h1 className="text-2xl font-bold mb-4">One Piece Card Not Found</h1>
        <Link href="/upload/onepiece" className="text-blue-500">
          Back to One Piece Upload
        </Link>
      </div>
    );
  }
  ...
  // 🎯 For One Piece cards: Build dvgGrading from conversational data
  const dvgGrading = {
    ...
    // 🆕 Always include sub_scores from conversational data for One Piece cards
  ...
  const cardInfo: Record<string, any> = {
    ...
    manufacturer: stripMarkdown(convInfo.manufacturer) || card.manufacturer_name || 'Bandai',
    ...
    op_card_type: stripMarkdown(convInfo.op_card_type) || card.op_card_type || null,
    op_card_color: stripMarkdown(convInfo.card_color) || card.op_card_color || null,
    op_card_power: convInfo.card_power ?? card.op_card_power ?? null,
    op_card_cost: convInfo.card_cost ?? card.op_card_cost ?? null,
    op_life: convInfo.life ?? card.op_life ?? null,
    op_counter: convInfo.counter_amount ?? card.op_counter ?? null,
    op_attribute: stripMarkdown(convInfo.attribute) || card.op_attribute || null,
    op_sub_types: stripMarkdown(convInfo.sub_types) || card.op_sub_types || null,
    op_variant_type: stripMarkdown(convInfo.variant_type) || card.op_variant_type || null,
    ...
    // One Piece variant features
    is_parallel: convInfo.is_parallel ?? (card.op_variant_type?.includes('parallel') ?? false),
    is_manga_art: convInfo.is_manga_art ?? (card.op_variant_type?.includes('manga') ?? false),
    is_alternate_art: convInfo.is_alternate_art ?? (card.op_variant_type === 'alternate_art' || false),
    is_sp: convInfo.is_sp ?? (card.op_variant_type === 'sp' || false),
    // Other fields
  ...
  // 🎯 One Piece cards: Map conversational data to expected structure
  ...
  // For One Piece cards, create gradingScale from conversational data
  const gradingScale: any = card.conversational_sub_scores ? {
  ...
  // 🎯 One Piece cards: Extract centering from conversational_grading JSON as primary source
  ...
  if (card.conversational_grading) {
    try {
      ...
      if (parsed.centering) {
        ...
        console.log('[One Piece Page] 🔧 Extracted centering from conversational_grading JSON:', parsedCenteringFromJSON);
      }
    } catch (e) {
      console.log('[One Piece Page] Could not parse conversational_grading for centering:', e);
    }
  }

  // 🎯 One Piece cards: Use parsed JSON first, then fall back to other sources
    ...
    <div className="dcm-report-page container mx-auto p-6 max-w-7xl" style={{ '--dcm-report-foil': reportFoil } as React.CSSProperties}>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          <Link href="/upload/onepiece" className="text-purple-600 hover:text-purple-800">
            ← Back to One Piece Upload
          </Link>
          <button
            onClick={() => {
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-6">
                ...
                {(card.conversational_image_confidence === 'C' || card.conversational_image_confidence === 'D') && (
                  <div className="mt-4 mx-auto max-w-md bg-amber-500/20 border border-amber-300/70 rounded-lg px-4 py-3 flex items-start gap-2 text-left">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <Link
                        href="/upload?category=One%20Piece"
              ...
              {(() => {
                ...
                return (
                  <div id="tour-download-buttons" className="flex flex-col md:flex-row items-center justify-between gap-4 my-6 px-4">
                    ...
                    {isOwner && <DownloadReportButton card={card} cardType={"onepiece" as any} showFounderEmblem={showFounderEmblem} showVipEmblem={showVipEmblem} showCardLoversEmblem={showCardLoversEmblem} labelStyle={labelStyle} customLabelConfig={activeConfig} />}
              ...
              <div id="tour-card-info" className="bg-white rounded-xl shadow-lg border-2 border-gray-200 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {(cardInfo.op_card_type || card.op_card_type) && (
                    <div className="space-y-1">
                  ...
                  {/* Color */}
                  {(cardInfo.op_card_color || card.op_card_color) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Color</p>
                      <div className="flex gap-1 flex-wrap">
                        {(cardInfo.op_card_color || card.op_card_color).split('/').map((color: string, idx: number) => {
                  ...
                  {(cardInfo.op_card_power || card.op_card_power) && (
                    <div className="space-y-1">
                  ...
                  {/* Cost (DON!!) */}
                  {(cardInfo.op_card_cost !== undefined && cardInfo.op_card_cost !== null) || (card.op_card_cost !== undefined && card.op_card_cost !== null) ? (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Cost (DON!!)</p>
                      <p className="font-semibold text-blue-600 text-lg">
                        {cardInfo.op_card_cost ?? card.op_card_cost}
                      </p>
                    </div>
                  ) : null}

                  {/* Life (Leaders only) */}
                  {(cardInfo.op_life !== undefined && cardInfo.op_life !== null) || (card.op_life !== undefined && card.op_life !== null) ? (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Life</p>
                      <p className="font-semibold text-red-600 text-lg">
                        {cardInfo.op_life ?? card.op_life}
                      </p>
                    </div>
                  ) : null}

                  {/* Counter */}
                  {(cardInfo.op_counter !== undefined && cardInfo.op_counter !== null) || (card.op_counter !== undefined && card.op_counter !== null) ? (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Counter</p>
                      <p className="font-semibold text-green-600">
                        +{cardInfo.op_counter ?? card.op_counter}
                      </p>
                    </div>
                  ) : null}

                  {/* Attribute (Slash, Strike, Ranged, etc.) */}
                  {(cardInfo.op_attribute || card.op_attribute) && (
                    <div className="space-y-1">
                  ...
                  {(cardInfo.op_sub_types || card.op_sub_types) && (
                    <div className="space-y-1">
                  ...
                  {/* Variant Type */}
                  {(cardInfo.op_variant_type || card.op_variant_type) && (
                    <div className="space-y-1">
                      ...
                      <span className="px-2 py-1 rounded-lg text-xs font-bold bg-gradient-to-r from-purple-500 to-pink-500 text-white">
                        {(cardInfo.op_variant_type || card.op_variant_type).replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                      </span>
                    </div>
                  )}
                ...
                {(() => {
                  ...
                  // Check if we have any special features to show (One Piece specific)
                  const hasSpecialFeatures = dvgGrading.rarity_features || cardInfo.serial_number ||
                    ...
                    cardInfo.op_variant_type || cardInfo.is_parallel || cardInfo.is_manga_art ||
                  ...
                  <div className="border-t pt-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {/* Parallel Art - One Piece specific */}
                      {(cardInfo.is_parallel || cardInfo.op_variant_type?.includes('parallel')) && (
                      ...
                      {/* Manga Art - One Piece specific */}
                      {(cardInfo.is_manga_art || cardInfo.op_variant_type?.includes('manga')) && !cardInfo.op_variant_type?.includes('parallel_manga') && (
                      ...
                      {/* Parallel Manga - One Piece specific (combines both) */}
                      {cardInfo.op_variant_type === 'parallel_manga' && (
                      ...
                      {/* SP (Special) Card - One Piece specific */}
                      {(cardInfo.is_sp || cardInfo.op_variant_type === 'sp') && (
                      ...
                      {/* Alternate Art - One Piece specific */}
                      {(cardInfo.is_alternate_art || cardInfo.op_variant_type === 'alternate_art') && (
                ...
                {/* One Piece Card Text Section - Abilities, Effects, Flavor Text */}
                  ...
                  return (
                    <OnePiecePriceLookup
                      ...
                      card={{
                        ...
                        rarity_or_variant: cardInfo.rarity || cardInfo.op_variant_type,
                  ...
                  <a
                    href={dcmPriceData?.priceChartingUrl || `https://www.pricecharting.com/search-products?q=${encodeURIComponent(
                      [
                        ...
                        cardInfo.card_id || cardInfo.card_number || card.card_number
                      ].filter(Boolean).join(' ')
                    )}&type=prices`}
                    target="_blank"
                    rel="noopener noreferrer"
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-gray-700">
                          ...
                          <ul className="list-disc list-inside space-y-0.5 text-gray-600">
                            ...
                            <li>Pre-filled One Piece TCG item specifics</li>
                    ...
                    <div className="p-8">
                      {(() => {
                        ...
                        if (isJSONReport && jsonData) {
                          ...
                          // Card Information Section - Enhanced for One Piece with Database API data
                          const renderCardInfo = () => {
                            // 🏴‍☠️ One Piece: Use conversational_card_info (has merged Database data) as primary source
                            ...
                            const manufacturer = apiInfo.manufacturer || info.manufacturer || 'Bandai';
                            const rarity = apiInfo.rarity_or_variant || apiInfo.rarity_tier || info.rarity_tier;

                            // One Piece-specific fields from database
                            const opCardType = apiInfo.op_card_type;
                            const opCardColor = apiInfo.card_color || apiInfo.op_card_color;
                            const opCardPower = apiInfo.card_power || apiInfo.op_card_power;
                            const opCardCost = apiInfo.card_cost || apiInfo.op_card_cost;
                            const opLife = apiInfo.life || apiInfo.op_life;
                            const opCounter = apiInfo.counter_amount || apiInfo.op_counter;
                            const opAttribute = apiInfo.attribute || apiInfo.op_attribute;
                            const opSubTypes = apiInfo.sub_types || apiInfo.op_sub_types;
                            const opVariantType = apiInfo.variant_type || apiInfo.op_variant_type;
                            ...
                            return (
                              <>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 mt-4">
                                  ...
                                  {opCardType && renderField('Card Type', opCardType)}
                                  {opCardColor && renderField('Color', opCardColor)}
                                  {opCardPower && renderField('Power', String(opCardPower))}
                                  {opCardCost !== undefined && opCardCost !== null && renderField('Cost (DON!!)', String(opCardCost))}
                                  {opLife !== undefined && opLife !== null && renderField('Life', String(opLife))}
                                  {opCounter !== undefined && opCounter !== null && renderField('Counter', `+${opCounter}`)}
                                  {opAttribute && renderField('Attribute', opAttribute)}
                                  {opSubTypes && renderField('Affiliations', opSubTypes)}
                                  {opVariantType && renderField('Variant', opVariantType.replace(/_/g, ' '))}
          ...
          {(() => {
            ...
            const href = outOfCredits ? '/credits' : '/upload?category=One%20Piece';
...
export default OnePieceCardDetails;