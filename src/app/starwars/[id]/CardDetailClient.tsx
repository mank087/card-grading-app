...
interface SportsCard {
  ...
  sw_card_type?: string | null;     // Leader, Character, Event, Stage
  sw_faction?: string | null;    // Red, Blue, Green, Purple, Black, Yellow (or multi-color)
  sw_card_type?: number | null;    // Power value (Characters and Leaders)
  sw_faction?: number | null;     // DON!! cost to play
  sw_era?: number | null;          // Life value (Leaders only)
  sw_era?: number | null;       // Counter value: 1000, 2000, or NULL
  sw_era?: string | null;     // Combat attribute: Slash, Strike, Ranged, Special, Wisdom
  sw_rarity?: string | null;     // Character traits/affiliations
  sw_card_type?: string | null;  // parallel, manga, parallel_manga, sp, alternate_art, etc.
...
export function StarWarsCardDetails() {
  ...
  const cardInfo = {
    ...
    sw_card_type: stripMarkdown(convInfo.sw_card_type) || card.sw_card_type || null,
    sw_faction: stripMarkdown(convInfo.card_color) || card.sw_faction || null,
    sw_card_type: convInfo.card_power ?? card.sw_card_type ?? null,
    sw_faction: convInfo.card_cost ?? card.sw_faction ?? null,
    sw_era: convInfo.life ?? card.sw_era ?? null,
    sw_era: convInfo.counter_amount ?? card.sw_era ?? null,
    ...
    is_parallel: convInfo.is_parallel ?? (card.sw_card_type?.includes('parallel') ?? false),
    is_manga_art: convInfo.is_manga_art ?? (card.sw_card_type?.includes('manga') ?? false),
    is_alternate_art: convInfo.is_alternate_art ?? (card.sw_card_type === 'alternate_art' ?? false),
    is_sp: convInfo.is_sp ?? (card.sw_card_type === 'sp' ?? false),
    // Other fields
              ...
              {(() => {
                ...
                return (
                  <div id="tour-download-buttons" className="flex flex-col md:flex-row items-center justify-between gap-4 my-6 px-4">
                    ...
                    {isOwner && <DownloadReportButton card={card} cardType="starwars" showFounderEmblem={showFounderEmblem} showVipEmblem={showVipEmblem} showCardLoversEmblem={showCardLoversEmblem} labelStyle={labelStyle} customLabelConfig={activeConfig} />}
                  ...
                  {(cardInfo.sw_card_type || card.sw_card_type) && (
                    <div className="space-y-1">
                      ...
                      <span className="px-2 py-1 rounded-lg text-xs font-bold bg-gradient-to-r from-purple-500 to-pink-500 text-white">
                        {(cardInfo.sw_card_type || card.sw_card_type).replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                      </span>
                    </div>
                  )}
                ...
                {(() => {
                  ...
                  <div className="border-t pt-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      {(cardInfo.is_parallel || cardInfo.sw_card_type?.includes('parallel')) && (
                      ...
                      {(cardInfo.is_manga_art || cardInfo.sw_card_type?.includes('manga')) && !cardInfo.sw_card_type?.includes('parallel_manga') && (
                  ...
                  return (
                    <OtherPriceLookup
                      card={{
                        ...
                        manufacturer: 'Topps',
                        rarity_or_variant: cardInfo.rarity || cardInfo.sw_card_type,
                        game_type: 'Star Wars',
