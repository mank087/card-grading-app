...
interface SportsAIGrading {
  ...
  // v3.1 top-level fields (conversational grading payloads)
...
interface SportsCard {
  ...
  // Card info fields (shared card-detail schema)
  ...
  // 🏴‍☠️ Star Wars database verification
  starwars_card_id?: string | null;
  starwars_reference_image?: string | null;
  starwars_database_match_confidence?: 'high' | 'medium' | 'low' | null;

  // Star Wars-specific fields
  sw_card_type?: string | number | null;     // Leader, Character, Event, Stage
  sw_faction?: string | number | null;    // Red, Blue, Green, Purple, Black, Yellow (or multi-color)
  sw_era?: string | number | null;          // Life value (Leaders only)
  sw_rarity?: string | null;     // Character traits/affiliations
...
function generateStructuredData(card: any, dvgGrading: any, cardUrl: string) {
  ...
  const productSchema = {
    ...
    category: 'Star Wars TCG Trading Cards',
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: fullCardName || 'Star Wars Card',
        item: cardUrl
      }
    ]
  };
...
export function StarWarsCardDetails() {
  ...
  const classicQrDataUrl = useClassicQrDataUrl(
    isClassicLabel && origin && cardId ? `${origin}/starwars/${cardId}` : null
  );
  ...
  // Fetch Star Wars Card Details using Star Wars-specific API
  const fetchStarWarsCardDetails = useCallback(async () => {
    if (!cardId) return;
    ...
    try {
      ...
      console.log(`[FRONTEND DEBUG] Calling Star Wars API endpoint: /api/starwars/${cardId}`);
      const res = await fetch(`/api/starwars/${cardId}?t=${Date.now()}${userParam}`); // Cache-busting
      console.log(`[FRONTEND DEBUG] Star Wars API response status: ${res.status}`);
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) {
          ...
          const retryWithBackoff = async (attempt: number = 1): Promise<void> => {
            ...
            try {
              console.log(`[FRONTEND DEBUG] Retry ${attempt}: Calling /api/starwars/${cardId}`);
              const retryRes = await fetch(`/api/starwars/${cardId}?t=${Date.now()}${userParam}`); // Cache-busting
  ...
  useEffect(() => {
    if (cardId) {
      console.log('Fetching Star Wars Card Details for:', cardId);
      fetchStarWarsCardDetails();
    }
  }, [cardId, fetchStarWarsCardDetails]);
  ...
  useEffect(() => {
    if (card && card.grade && !hasTrackedGradeComplete.current && typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'grade_card_complete', {
        card_category: card.category || 'Star Wars',
        grade: card.grade
      });
  ...
  const regradeCard = async () => {
    ...
    try {
      ...
      queueId = addToQueue({
        cardId: cardId,
        category: 'starwars',
        categoryLabel: 'Star Wars Card (Re-grade)',
        frontImageUrl: card.front_url,
        status: 'processing',
        resultUrl: `/starwars/${cardId}`
      });
      console.log('[REGRADE] Added to queue:', queueId);

      // Call Star Wars API with force_regrade parameter to bypass cache
      console.log('[REGRADE] Forcing fresh grading for Star Wars card:', cardId);

      // Re-grading can take 30-60 seconds, so use a longer timeout
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 120000); // 120 second timeout

      const res = await fetch(`/api/starwars/${cardId}?force_regrade=true&t=${Date.now()}`, {
  ...
  // 🎯 For Star Wars cards: Build dvgGrading from conversational data
  const dvgGrading = {
    ...
    // 🆕 Always include sub_scores from conversational data for Star Wars cards
  ...
  const cardInfo = {
    ...
    manufacturer: stripMarkdown(convInfo.manufacturer) || card.manufacturer_name || 'Topps',
    ...
    // (duplicate keys removed — with duplicate object keys the last value wins at
    // runtime, so only the final sw_card_type / sw_faction / sw_era entries applied)
    sw_faction: convInfo.card_cost ?? card.sw_faction ?? null,
    sw_era: stripMarkdown(convInfo.attribute) || card.sw_era || null,
    sw_rarity: stripMarkdown(convInfo.sub_types) || card.sw_rarity || null,
    sw_card_type: stripMarkdown(convInfo.variant_type) || card.sw_card_type || null,
    ...
    // Star Wars variant features
    is_parallel: convInfo.is_parallel ?? ((card.sw_card_type as string | null | undefined)?.includes('parallel') ?? false),
    is_manga_art: convInfo.is_manga_art ?? ((card.sw_card_type as string | null | undefined)?.includes('manga') ?? false),
    is_alternate_art: convInfo.is_alternate_art ?? (card.sw_card_type === 'alternate_art'),
    is_sp: convInfo.is_sp ?? (card.sw_card_type === 'sp'),
    // Other fields
  ...
  // 🎯 Star Wars cards: Map conversational data to expected structure
  ...
  // For Star Wars cards, create gradingScale from conversational data
  ...
  // 🎯 Star Wars cards: Extract centering from conversational_grading JSON as primary source
  ...
  if (card.conversational_grading) {
    try {
      ...
      if (parsed.centering) {
        ...
        console.log('[Star Wars Page] 🔧 Extracted centering from conversational_grading JSON:', parsedCenteringFromJSON);
      }
    } catch (e) {
      console.log('[Star Wars Page] Could not parse conversational_grading for centering:', e);
    }
  }

  // 🎯 Star Wars cards: Use parsed JSON first, then fall back to other sources
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl w-full">
            ...
            >
              ...
              <div
                ...
                onClick={() => openZoomModal(card.front_url, "Star Wars card front", "Card Front - Click for detailed view")}
              >
                <Image
                  src={card.front_url}
                  alt="Star Wars card front"
                  width={400}
                  height={560}
                  className="w-full"
                  priority
            ...
            >
              ...
              <div
                className="relative cursor-pointer transition-transform hover:scale-[1.02]"
                onClick={() => openZoomModal(card.back_url, "Star Wars card back", "Card Back - Click for detailed view")}
              >
                <Image
                  src={card.back_url}
                  alt="Star Wars card back"
                  width={400}
                  height={560}
                  className="w-full"
                />
            ...
            <div className="space-y-6">
              ...
              {(() => {
                ...
                return (
                  <div id="tour-download-buttons" className="flex flex-col md:flex-row items-center justify-between gap-4 my-6 px-4">
                    ...
                    {isOwner && <DownloadReportButton card={card} cardType={"starwars" as any} showFounderEmblem={showFounderEmblem} showVipEmblem={showVipEmblem} showCardLoversEmblem={showCardLoversEmblem} labelStyle={labelStyle} customLabelConfig={activeConfig} />}
              ...
              <div id="tour-card-info" className="bg-white rounded-xl shadow-lg border-2 border-gray-200 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {(cardInfo.sw_card_type || card.sw_card_type) && (
                    <div className="space-y-1">
                  ...
                  {/* Color */}
                  {(cardInfo.sw_faction || card.sw_faction) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Color</p>
                      <div className="flex gap-1 flex-wrap">
                        {(cardInfo.sw_faction || card.sw_faction).split('/').map((color: string, idx: number) => {
                  ...
                  {/* Cost (DON!!) */}
                  {(cardInfo.sw_faction !== undefined && cardInfo.sw_faction !== null) || (card.sw_faction !== undefined && card.sw_faction !== null) ? (
                    <div className="space-y-1">
                  ...
                  {/* Life (Leaders only) */}
                  {(cardInfo.sw_era !== undefined && cardInfo.sw_era !== null) || (card.sw_era !== undefined && card.sw_era !== null) ? (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Life</p>
                      <p className="font-semibold text-red-600 text-lg">
                        {cardInfo.sw_era ?? card.sw_era}
                      </p>
                    </div>
                  ) : null}
                  ...
                  {(cardInfo.sw_rarity || card.sw_rarity) && (
                    <div className="space-y-1">
                  ...
                  {(cardInfo.sw_card_type || card.sw_card_type) && (
                    <div className="space-y-1">
                      ...
                      <span className="px-2 py-1 rounded-lg text-xs font-bold bg-gradient-to-r from-purple-500 to-pink-500 text-white">
                        {((cardInfo.sw_card_type || card.sw_card_type) as string | null | undefined)?.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                      </span>
                    </div>
                  )}
                ...
                {(() => {
                  ...
                  // Check if we have any special features to show (Star Wars specific)
                  const hasSpecialFeatures = dvgGrading.rarity_features || cardInfo.serial_number ||
                    ...
                    cardInfo.sw_card_type || cardInfo.is_parallel || cardInfo.is_manga_art ||
                  ...
                  <div className="border-t pt-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {/* Parallel Art - Star Wars specific */}
                      {(cardInfo.is_parallel || (cardInfo.sw_card_type as string | null)?.includes('parallel')) && (
                      ...
                      {/* Manga Art - Star Wars specific */}
                      {(cardInfo.is_manga_art || (cardInfo.sw_card_type as string | null)?.includes('manga')) && !(cardInfo.sw_card_type as string | null)?.includes('parallel_manga') && (
                      ...
                      {/* Parallel Manga - Star Wars specific (combines both) */}
                      {cardInfo.sw_card_type === 'parallel_manga' && (
                      ...
                      {/* SP (Special) Card - Star Wars specific */}
                      {(cardInfo.is_sp || cardInfo.sw_card_type === 'sp') && (
                      ...
                      {/* Alternate Art - Star Wars specific */}
                      {(cardInfo.is_alternate_art || cardInfo.sw_card_type === 'alternate_art') && (
                ...
                {/* Star Wars Card Text Section - Abilities, Effects, Flavor Text */}
                  ...
                  return (
                    <OtherPriceLookup
                      card={{
                        ...
                        manufacturer: 'Topps',
                        rarity_or_variant: (cardInfo as any).rarity || cardInfo.sw_card_type,
                        game_type: 'Star Wars',
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-gray-700">
                          ...
                          <ul className="list-disc list-inside space-y-0.5 text-gray-600">
                            ...
                            <li>Pre-filled Star Wars TCG item specifics</li>
                    ...
                    <div className="p-8">
                      {(() => {
                        ...
                        if (isJSONReport && jsonData) {
                          ...
                          // Card Information Section - Enhanced for Star Wars with Database API data
                          const renderCardInfo = () => {
                            // 🏴‍☠️ Star Wars: Use conversational_card_info (has merged Database data) as primary source
                            ...
                            const manufacturer = apiInfo.manufacturer || info.manufacturer || 'Topps';
                            const rarity = apiInfo.rarity_or_variant || apiInfo.rarity_tier || info.rarity_tier;

                            // Star Wars-specific fields from database
                            const swCardType = apiInfo.sw_card_type || apiInfo.card_type;
                            const swFaction = apiInfo.sw_faction || apiInfo.faction;
                            const swEra = apiInfo.sw_era || apiInfo.era;
                            const swRarity = apiInfo.sw_rarity || apiInfo.rarity_or_variant;
                            ...
                            return (
                              <>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 mt-4">
                                  ...
                                  {swCardType && renderField('Card Type', swCardType)}
                                  {swFaction && renderField('Faction', swFaction)}
                                  {swEra && renderField('Era', swEra)}
                                  {swRarity && renderField('Variant', swRarity)}
        ...
        <EditCardLabelModal
          ...
          onSaved={async () => {
            try {
              const session = getStoredSession();
              const userParam = session?.user?.id ? `&user_id=${session.user.id}` : '';
              const res = await fetch(`/api/starwars/${card.id}?t=${Date.now()}${userParam}`);
              if (res.ok) {
                const data = await res.json();
                setCard(data);
              }
...
export default StarWarsCardDetails;