...
interface SportsAIGrading {
  ...
  // Parallel Processing v2.3 - front/back specific feedback + OCR
  ...
  // v3.1 top-level fields
  category_scores?: Record<string, any> | null;
  centerings_used?: Record<string, any> | null;
  "Centering_Measurements"?: Record<string, any> | null;
}
...
interface SportsCard {
  ...
  // Final grade (DB column)
  grade?: number | string | null;

  // Category-specific DB fields
  ...
  ink_cost?: string | number | null;
  willpower?: string | number | null;
  strength?: string | number | null;

  // Card owner's emblem/badge settings
  ...
  // Conversational grading structured columns
  ...
  // User-reported condition report
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: fullCardName || 'Collectible Card',
        item: cardUrl
      }
    ]
  };
...
export function OtherCardDetails() {
  ...
  // 🔄 Re-grading state - stores image URL for loading screen
  ...
  useEffect(() => {
    ...
    const qrUrl = origin && cardId ? `${origin}/other/${cardId}` : '';
  ...
  const regradeCard = async () => {
    ...
    try {
      ...
      queueId = addToQueue({
        cardId: cardId,
        category: 'other',
        categoryLabel: 'Other Card (Re-grade)',
        frontImageUrl: card.front_url,
        status: 'processing',
        resultUrl: `/other/${cardId}`
      });
      console.log('[REGRADE] Added to queue:', queueId);

      // Call Other API with force_regrade parameter to bypass cache
      console.log('[REGRADE] Forcing fresh grading for Other card:', cardId);
  ...
  // Type for the unified card-info object below. Some fields (authentic, language,
  // Lorcana stats, etc.) are read later in the page but only populated for certain
  // AI versions/categories — they are declared optional here so those reads type-check.
  type UnifiedCardInfo = {
    card_name?: any; player_or_character?: any; set_name?: any; set_era?: any; year?: any;
    manufacturer?: any; card_number?: any; sport_or_category?: any; serial_number?: any;
    rookie_or_first?: any; subset?: any; rarity_tier?: any; autographed?: any; memorabilia?: any;
    card_front_text?: any; card_back_text?: any; pokemon_type?: any; pokemon_stage?: any; hp?: any;
    card_type?: any; card_date?: any; special_features?: any; front_text?: any; back_text?: any;
    facsimile_autograph?: any; official_reprint?: any;
    authentic?: any; rarity_or_variant?: any; language?: any; flavor_text?: any; collector_number?: any;
    character_version?: any; lorcana_card_type?: any; ink_color?: any; ink_cost?: any; willpower?: any;
    strength?: any; game_type?: any;
  };
  const cardInfo: UnifiedCardInfo = {
    ...
    // manufacturer: superseded by the OTHER-SPECIFIC manufacturer key below (duplicate key — later key wins at runtime)
    ...
    // autographed: superseded by the OTHER-SPECIFIC autographed key below (duplicate key — later key wins at runtime)
    // memorabilia: superseded by the OTHER-SPECIFIC memorabilia key below (duplicate key — later key wins at runtime)
  ...
  if (card.conversational_grading) {
    try {
      ...
      if (parsed.centering) {
        ...
        console.log('[Other Page] 🔧 Extracted centering from conversational_grading JSON:', parsedCenteringFromJSON);
      }
    } catch (e) {
      console.log('[Other Page] Could not parse conversational_grading for centering:', e);
    }
  }

  // 🎯 Other cards: Use parsed JSON first, then fall back to other sources
  const centeringData: any = parsedCenteringFromJSON ||
              ...
              {(() => {
                ...
                return (
                  <div id="tour-download-buttons" className="flex flex-col md:flex-row items-center justify-between gap-4 my-6 px-4">
                    ...
                    {isOwner && <DownloadReportButton card={card} cardType="other" showFounderEmblem={showFounderEmblem} showVipEmblem={showVipEmblem} showCardLoversEmblem={showCardLoversEmblem} labelStyle={labelStyle} customLabelConfig={activeConfig} />}
                  ...
                  return (
                    <OtherPriceLookup
                      card={{
                        ...
                        card_set: (extractEnglishForSearch(cardInfo.set_name) || card.card_set) as string | undefined,
        ...
        <EditCardLabelModal
          ...
          onSaved={async () => {
            try {
              ...
              const res = await fetch(`/api/other/${card.id}?t=${Date.now()}${userParam}`);
              if (res.ok) {
                const data = await res.json();
                setCard(data);
              }
