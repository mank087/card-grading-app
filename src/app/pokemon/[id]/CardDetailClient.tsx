...
import { generatePokemonEbaySearchUrl, generatePokemonEbaySoldListingsUrl, type CardData } from "@/lib/ebayUtils";
...
interface SportsAIGrading {
  ...
  // Legacy/v3.1 top-level fields (present in older AI grading payloads)
...
interface SportsCard {
  ...
  // Additional DB row fields used by this page
  grade?: number | null;
  card_type?: string | null;
  rarity_description?: string | null;
  autographed?: boolean | null;
  holofoil?: boolean | null;
  tcgplayer_url?: string | null;
  pokemon_api_data?: any;
  ...
  // Conversational grading extras (JSONB columns)
  ...
  // Card owner badge/emblem preferences
...
export function PokemonCardDetails() {
  ...
  const classicQrDataUrl = useClassicQrDataUrl(
    isClassicLabel && origin && cardId ? `${origin}/pokemon/${cardId}` : null
  );
  ...
  useEffect(() => {
    if (cardId) {
      console.log('Fetching Pokemon card details for:', cardId);
      fetchPokemonCardDetails();
    }
  }, [cardId, fetchPokemonCardDetails]);
  ...
  const regradeCard = async () => {
    ...
    try {
      ...
      queueId = addToQueue({
        cardId: cardId,
        category: 'pokemon',
        categoryLabel: 'Pokemon Card (Re-grade)',
        frontImageUrl: card.front_url,
        status: 'processing',
        resultUrl: `/pokemon/${cardId}`
      });
      console.log('[REGRADE] Added to queue:', queueId);

      // Call Pokemon API with force_regrade parameter to bypass cache
      console.log('[REGRADE] Forcing fresh grading for Pokemon card:', cardId);

      // Re-grading can take 30-60 seconds, so use a longer timeout
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 120000); // 120 second timeout

      const res = await fetch(`/api/pokemon/${cardId}?force_regrade=true&t=${Date.now()}`, {
  ...
  if (!card) {
    return (
      <div className="text-center p-8">
        <h1 className="text-2xl font-bold mb-4">Pokemon Card Not Found</h1>
        <Link href="/upload/pokemon" className="text-blue-500">
  ...
  // 🎯 For Pokemon cards: Build dvgGrading from conversational data
  const dvgGrading = {
    ...
    // 🆕 Always include sub_scores from conversational data for Pokemon cards
  ...
  const cardInfo: {
    ...
    hp: any;
    card_type: any;
    // Optional fields read by legacy sections (populated on other card types)
  ...
  const currentUrl = `${origin}/pokemon/${cardId}`;
  ...
  // 🎯 Pokemon cards: Map conversational data to expected structure
  ...
  // For Pokemon cards, create gradingScale from conversational data
  ...
  // 🎯 Pokemon cards: Extract centering from conversational_grading JSON as primary source
  ...
  if (card.conversational_grading) {
    try {
      ...
      if (parsed.centering) {
        ...
        console.log('[Pokemon Page] 🔧 Extracted centering from conversational_grading JSON:', parsedCenteringFromJSON);
      }
    } catch (e) {
      console.log('[Pokemon Page] Could not parse conversational_grading for centering:', e);
    }
  }

  // 🎯 Pokemon cards: Use parsed JSON first, then fall back to other sources
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-600 space-y-0.5">
                        <p>F: <span className="font-semibold text-blue-700">{safeToFixed(subScores?.centering?.front_score ?? subScores?.centering?.front ?? 0)}</span> | B: <span className="font-semibold text-blue-700">{safeToFixed(subScores?.centering?.back_score ?? subScores?.centering?.back ?? 0)}</span></p>
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-600 space-y-0.5">
                        <p>F: <span className="font-semibold text-green-700">{safeToFixed(subScores?.corners?.front_score ?? subScores?.corners?.front ?? 0)}</span> | B: <span className="font-semibold text-green-700">{safeToFixed(subScores?.corners?.back_score ?? subScores?.corners?.back ?? 0)}</span></p>
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-600 space-y-0.5">
                        <p>F: <span className="font-semibold text-purple-700">{safeToFixed(subScores?.edges?.front_score ?? subScores?.edges?.front ?? 0)}</span> | B: <span className="font-semibold text-purple-700">{safeToFixed(subScores?.edges?.back_score ?? subScores?.edges?.back ?? 0)}</span></p>
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-600 space-y-0.5">
                        <p>F: <span className="font-semibold text-amber-700">{safeToFixed(subScores?.surface?.front_score ?? subScores?.surface?.front ?? 0)}</span> | B: <span className="font-semibold text-amber-700">{safeToFixed(subScores?.surface?.back_score ?? subScores?.surface?.back ?? 0)}</span></p>
              ...
              <div id="tour-card-info" className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-lg p-6 border-2 border-gray-200">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-6">
                  <div className="space-y-1">
                    ...
                    {(() => {
                      const cardName = cardInfo.card_name || cardInfo.player_or_character || card.pokemon_featured || card.featured || 'Unknown';
                      const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(cardName);

                      if (hasJapanese) {
                        // Try to split if both Japanese and English exist
                        ...
                        if (japanesePart && englishPart) {
                          return (
                            <div>
                              <p className="font-bold text-gray-900 text-lg font-noto-sans-jp">
                                {japanesePart.trim()}
                              </p>
                      ...
                      return <p className="font-bold text-gray-900 text-lg">{cardName}</p>;
                    })()}
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-500 text-xs uppercase tracking-wide">Set Name</p>
                    {(() => {
                      const setNameText = cardInfo.set_name || cardInfo.set_era || card.card_set || 'Unknown';
                      const isEra = !cardInfo.set_name && !card.card_set && cardInfo.set_era;
                      const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(setNameText);

                      if (hasJapanese) {
                        const parts = setNameText.split(/[/()（）]/);
                        const japanesePart = parts.find((p: string) => /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(p));
                        const englishPart = parts.find((p: string) => p.trim() && !/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(p));
                          ...
                          return (
                            <div>
                              <p className="font-semibold text-gray-900 font-noto-sans-jp">
                                {japanesePart.trim()}
                                {isEra && <span className="text-xs text-gray-500 ml-2 font-sans">(Era)</span>}
                              </p>
                              <p className="text-xs text-gray-600 mt-0.5">{englishPart.trim()}</p>
                            </div>
                          );
                        }
                        return (
                      ...
                      return (
                        <p className="font-semibold text-gray-900">
                          {setNameText}
                          {isEra && <span className="text-xs text-gray-500 ml-2">(Era)</span>}
                        </p>
                      );
                    })()}
                  </div>
                  <div className="space-y-1">
                    ...
                    <p className="font-semibold text-gray-900">{cardInfo.card_number || card.card_number || 'N/A'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-500 text-xs uppercase tracking-wide">Year</p>
                    <p className="font-semibold text-gray-900">{cardInfo.year || card.release_date || 'Unknown'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-500 text-xs uppercase tracking-wide">Rarity</p>
                    {(() => {
                      const rarityText = cardInfo.rarity_tier || card.rarity_tier || card.rarity_description || 'N/A';
                      const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(rarityText);

                      if (hasJapanese) {
                        const parts = rarityText.split(/[/()（）]/);
                        const japanesePart = parts.find((p: string) => /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(p));
                        const englishPart = parts.find((p: string) => p.trim() && !/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(p));
                        ...
                        if (japanesePart && englishPart) {
                          return (
                            <div>
                              <p className="font-semibold text-gray-900 font-noto-sans-jp">{japanesePart.trim()}</p>
                              <p className="text-xs text-gray-600 mt-0.5">{englishPart.trim()}</p>
                            </div>
                          );
                        }
                        return <p className="font-semibold text-gray-900 font-noto-sans-jp">{rarityText}</p>;
                      }

                      return <p className="font-semibold text-gray-900">{rarityText}</p>;
                    })()}
                  </div>
                  {/* Pokemon-specific fields with bilingual support */}
                  {(cardInfo.pokemon_type || card.pokemon_type) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Type</p>
                      {(() => {
                        const typeText = cardInfo.pokemon_type || card.pokemon_type;
                        const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(typeText);
                        ...
                        if (hasJapanese) {
                          ...
                          return <p className="font-semibold text-gray-900 font-noto-sans-jp">{typeText}</p>;
                        }

                        return <p className="font-semibold text-gray-900">{typeText}</p>;
                      })()}
                    </div>
                  )}
                  {(cardInfo.pokemon_stage || card.pokemon_stage) && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Stage</p>
                      {(() => {
                        const stageText = cardInfo.pokemon_stage || card.pokemon_stage;
                        const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(stageText);

                        if (hasJapanese) {
                          const parts = stageText.split(/[/()（）]/);
                          const japanesePart = parts.find((p: string) => /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(p));
                          const englishPart = parts.find((p: string) => p.trim() && !/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(p));
                          ...
                          return <p className="font-semibold text-gray-900 font-noto-sans-jp">{stageText}</p>;
                        }

                        return <p className="font-semibold text-gray-900">{stageText}</p>;
                      })()}
                    </div>
                  )}
                  {(cardInfo.hp || card.hp) && (
                    <div className="space-y-1">
                      ...
                      <p className="font-semibold text-gray-900">{cardInfo.hp || card.hp}</p>
                    </div>
                  )}
                  {/* 🎯 v3.2: Subset field with bilingual support */}
                  {cardInfo.subset && (
                    <div className="space-y-1">
                      <p className="text-gray-500 text-xs uppercase tracking-wide">Subset/Variant</p>
                      {(() => {
                        const subsetText = cardInfo.subset;
                        const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(subsetText);

                        if (hasJapanese) {
                          const parts = subsetText.split(/[/()（）]/);
                          const japanesePart = parts.find((p: string) => /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(p));
                          const englishPart = parts.find((p: string) => p.trim() && !/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(p));
                          ...
                          return <p className="font-semibold text-gray-900 font-noto-sans-jp">{subsetText}</p>;
                        }

                        return <p className="font-semibold text-gray-900">{subsetText}</p>;
                      })()}
                    </div>
                  )}
                </div>

                {/* Card Text Section - Front card text (abilities, attacks) with Japanese/English support */}
                {(cardInfo.card_front_text || cardInfo.card_back_text) && (
                  <div className="border-t pt-5 mt-5">
                    <div className="flex items-center gap-2 mb-3">
                      <h3 className="text-lg font-bold text-gray-800">Card Text</h3>
                      {(() => {
                        // Detect if text contains Japanese characters
                        const hasJapanese = (text: string | null | undefined) => {
                          if (!text) return false;
                          return /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(text);
                        };
                        const isJapanese = hasJapanese(cardInfo.card_front_text) || hasJapanese(cardInfo.card_back_text);
                        return isJapanese ? (
                          <span className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded-full font-semibold">🇯🇵 Japanese Card</span>
                        ) : null;
                      })()}
                    </div>

                    {/* Front Card Text (Abilities, Attacks, etc.) */}
                    {cardInfo.card_front_text && (
                      <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-lg p-4 border border-purple-200 mb-3">
                        <p className="text-sm font-semibold text-purple-700 mb-2">Card Front - Abilities & Attacks</p>
                        <div className="text-gray-800 leading-relaxed whitespace-pre-line space-y-2">
                          {(() => {
                            const text = cardInfo.card_front_text;
                            // Check if contains Japanese characters
                            const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(text);

                            if (hasJapanese) {
                              // Try to split Japanese and English if both exist
                              ...
                              if (japaneseLines.length > 0 && englishLines.length > 0) {
                                // Both languages detected - show side by side
                                return (
                                  <div className="grid md:grid-cols-2 gap-4">
                                    <div>
                                      <p className="text-xs font-semibold text-purple-600 mb-2">🇯🇵 Japanese</p>
                                      <div className="text-base leading-relaxed font-noto-sans-jp">
                                        {japaneseLines.join('\n')}
                                      </div>
                                    </div>
                                    <div>
                                      <p className="text-xs font-semibold text-purple-600 mb-2">🇺🇸 English Translation</p>
                              ...
                              } else if (japaneseLines.length > 0) {
                                // Japanese only
                                return (
                                  <div>
                                    <p className="text-xs font-semibold text-purple-600 mb-2">🇯🇵 Japanese Text</p>
                                    <div className="text-base leading-relaxed font-noto-sans-jp">
                                      {text}
                                    </div>
                                    <p className="text-xs text-gray-500 mt-3 italic">English translation not available</p>
                                  </div>
                                );
                              }
                            }

                            // English only or no special formatting needed
                            return <div>{text}</div>;
                          })()}
                        </div>
                      </div>
                    ...
                    {/* Back Card Text (Pokedex entry) */}
                    {cardInfo.card_back_text && (
                      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                        <p className="text-sm font-semibold text-blue-700 mb-2">Pokedex Entry</p>
                        <div className="text-gray-800 leading-relaxed">
                          {(() => {
                            const text = cardInfo.card_back_text;
                            const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(text);
                              ...
                              if (japaneseLines.length > 0 && englishLines.length > 0) {
                                return (
                                  <div className="space-y-3">
                                    <div>
                                      <p className="text-xs font-semibold text-blue-600 mb-1">🇯🇵 Japanese</p>
                                      <p className="text-base italic font-noto-sans-jp">
                                        "{japaneseLines.join(' ')}"
                                      </p>
                                    </div>
                                    <div>
                                      <p className="text-xs font-semibold text-blue-600 mb-1">🇺🇸 English</p>
              ...
              <div className="bg-gradient-to-br from-white to-blue-50 rounded-xl border-2 border-blue-200 p-6 shadow-lg">
                ...
                <div className="mb-6">
                  {(() => {
                    ...
                    const CenteringDial = ({ lrRatio, tbRatio, label }: { lrRatio: { left: number; right: number }; tbRatio: { left: number; right: number }; label: string }) => {
                      const quality = getQualityAssessment(lrRatio, tbRatio);
                      const lrDiff = Math.abs(lrRatio.left - 50);
                      const tbDiff = Math.abs(tbRatio.left - 50);
                      const worstDiff = Math.max(lrDiff, tbDiff);

                      // Map difference to gauge angle (0° = perfect center, 90° = worst)
                      const angle = Math.min(worstDiff * 9, 90); // Max 90 degrees
                      const needleRotation = -90 + angle; // Start at -90° (left), rotate right

                      return (
                        <div className="flex flex-col items-center gap-2 mb-2">
                          <svg width="120" height="75" viewBox="0 0 120 75" className="drop-shadow-md" preserveAspectRatio="xMidYMid meet">
                            {/* Background arc zones */}
                            <path
                              d="M 20 60 A 40 40 0 0 1 60 20"
                              fill="none"
                              stroke="#22c55e"
                              strokeWidth="12"
                              opacity="0.3"
                            ...
                            />
                            <path
                              d="M 85 42 A 40 40 0 0 1 100 60"
                              fill="none"
                              stroke="#ef4444"
                              strokeWidth="12"
                              opacity="0.3"
                            ...
                            {/* Threshold markers */}
                            <line x1="60" y1="60" x2="60" y2="25" stroke="#64748b" strokeWidth="1.5" opacity="0.5" />
                            <line x1="60" y1="60" x2="80" y2="35" stroke="#64748b" strokeWidth="1.5" opacity="0.5" />
                            ...
                            {/* Center dot */}
                            <circle cx="60" cy="60" r="4" fill={quality.color} />

                            {/* Label */}
                            <text x="60" y="72" textAnchor="middle" fontSize="10" fill="#475569" fontWeight="600">
                              {quality.text}
                            </text>
                          </svg>
                    ...
                    return (
                      <div className="space-y-6">
                        ...
                        <div className="flex flex-col lg:flex-row items-start justify-center gap-6 lg:gap-8">
                          ...
                          {/* Back Side */}
                          <div className="flex flex-col items-center gap-4 w-full lg:w-auto lg:max-w-md">
                            {/* Back Label - Purple DCM Gradient */}
                            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-lg px-6 py-2 shadow-md">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-gray-700">
                          ...
                          <ul className="list-disc list-inside space-y-0.5 text-gray-600">
                            ...
                            <li>Pre-filled Pokémon TCG item specifics</li>
                            ...
                            return (
                              <>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 mt-4">
                                  ...
                                  {renderField('Pokemon Stage', info.pokemon_stage)}
                                  {renderField('Pokemon Type', info.pokemon_type)}
                                  {renderField('HP', info.hp)}
