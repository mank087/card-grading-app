 ...
 *
 * It is a copy of the Pokemon adapter with only the things MTG genuinely does
 * differently changed — the pricing component, the marketplace links (four,
 * including Scryfall), the structured-data strings, the upload/retake routes
 * and the card-info slot. Every difference is marked MTG below so the next
 * category can see at a glance which lines are the adapter axis and which are
 * boilerplate.
 */
...
import MtgCardInfo, {
  MtgFeatureBadges,
  hasMtgFeatures,
} from '@/components/card-detail/categories/MtgCardInfo';
...
/** MTG: the Scryfall/PriceCharting lookup, not the Pokemon one (Phase 0 §4). */
...
/** MTG: the MTG-specific eBay builders. */
import {
  generateMTGEbaySearchUrl,
  generateMTGEbaySoldListingsUrl,
  type CardData,
} from '@/lib/ebayUtils';
import {
...
export function MTGCardDetailsV2() {
  const params = useParams<{ id: string }>();
  const cardId = typeof params?.id === 'string' ? params.id : '';

  const detail = useCardDetail('mtg', cardId);
  const card = detail.card;

  // The live PriceCharting lookup, reported by MTGPriceLookup in the Market
  ...
  /**
   * MTG: the legacy `cardInfo` object (mtg CardDetailClient.tsx 2628-2698).
   * Naming the category matters — MTG reads the model JSON BEFORE the database
   * columns, refuses to append a frame treatment to the set name, and falls
   * back to 'Wizards of the Coast' for the manufacturer. `buildCardInfo`
   * switches on exactly this argument.
   */
  const cardInfo = useMemo(() => buildCardInfo(card, 'mtg'), [card]);

  // Legacy's rule, verbatim in intent (mtg CardDetailClient.tsx 5430): only a
  ...
  /**
   * MTG: five links where Pokemon has four. The two eBay payloads are legacy's
   * field for field (5527-5580) — including `category: 'MTG'`, the collector
   * number, the expansion code, `is_foil` and the language — and the Scryfall
   * link is the same verified-card link legacy prints (5584-5605), present
   * only when the row carries an id.
   */
  const marketplaceLinks = () => {
    ...
    const collectorNumber = cardInfo.collector_number || cardInfo.card_number || c.card_number;
    const expansionCode = cardInfo.expansion_code || c.expansion_code;
    const isFoil = cardInfo.is_foil || c.is_foil || false;
    const subset = extractEnglishForSearch(cardInfo.subset) || extractEnglishForSearch(c.subset);
    ...
    // Legacy 5551/5580 adds the graded whole number to the eBay payloads only.
    ...
    // Legacy 5608-5620: the PriceCharting product page when the lookup found
    // one, otherwise a search on the card and set name.
    ...
    const scryfallId = c.mtg_api_id || c.scryfall_id || (cardInfo as any).scryfall_id;

    const links: Array<[string, string, string]> = [
      ['TCGPlayer', tcgplayerUrl, setName && setName !== 'Unknown' ? setName : 'Search listings'],
      ['eBay', generateMTGEbaySearchUrl(ebaySearchCard), 'Active listings'],
      ['eBay sold', generateMTGEbaySoldListingsUrl(ebaySearchCard), 'Price history'],
    ];
    if (scryfallId) {
      links.push([
        'Scryfall',
        `https://scryfall.com/card/${scryfallId}`,
        c.mtg_api_verified ? 'Verified card' : 'Card database',
      ]);
    }
    links.push(['PriceCharting', priceChartingUrl, 'Market data']);
  ...
  // MTG: the four strings come from the legacy mtg generateStructuredData
  // (1409-1530); the brand fallback is legacy's own 'Wizards of the Coast'.
  const structuredData = card
    ? buildCardStructuredData(
        card,
        `${typeof window !== 'undefined' ? window.location.origin : 'https://dcmgrading.com'}/mtg/${cardId}`,
        {
          fallbackBrand: 'Wizards of the Coast',
          productCategory: 'Magic: The Gathering Trading Cards',
          breadcrumbName: 'MTG Cards',
          breadcrumbUrl: 'https://dcmgrading.com/upload/mtg',
          fallbackCardName: 'MTG Card',
        },
      )
    : null;
  ...
  return (
    ...
    <CardDetailShell
      ...
      cardId={cardId}
      backHref="/collection"
      uploadHref="/upload/mtg"
      retakeHref="/upload?category=MTG"
      ebayCardType="mtg"
      detail={detail}
      vm={vm}
        ...
        card ? (
          <MTGPriceLookup
            ...
            card={{
              ...
              player_or_character: cardInfo.player_or_character || (card as any).featured || undefined,
              set_name: cardInfo.set_name || (card as any).card_set,
              collector_number:
                cardInfo.collector_number || cardInfo.card_number || (card as any).card_number,
              expansion_code: cardInfo.expansion_code || (card as any).expansion_code || undefined,
              ...
              rarity_or_variant:
                cardInfo.rarity_or_variant || (cardInfo as any).rarity || (card as any).mtg_rarity,
      ...
      renderCategoryCardInfo={(ctx) => <MtgCardInfo card={ctx.card} cardInfo={cardInfo} />}
      /* MTG: foil, Scryfall rarity, double-faced and the five frame flags. */
      renderCategoryBadges={() => <MtgFeatureBadges card={card} cardInfo={cardInfo} />}
      hasCategoryFeatures={hasMtgFeatures(card, cardInfo)}
    />
    </>
  );
}
