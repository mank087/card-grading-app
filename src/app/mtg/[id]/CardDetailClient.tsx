...
export function MTGCardDetails() {
  ...
  useEffect(() => {
    ...
    const heritageQrUrl = origin && cardId ? `${origin}/mtg/${cardId}` : '';
  ...
  useEffect(() => {
    if (card && card.grade && !hasTrackedGradeComplete.current && typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'grade_card_complete', {
        card_category: card.category || 'MTG',
        grade: card.grade
      });
  ...
  const regradeCard = async () => {
    ...
    try {
      ...
      queueId = addToQueue({
        cardId: cardId,
        category: 'mtg',
        categoryLabel: 'MTG Card (Re-grade)',
        frontImageUrl: card.front_url,
        status: 'processing',
        resultUrl: `/mtg/${cardId}`
      });
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-6">
              ...
              {(() => {
                ...
                return (
                  <div id="tour-download-buttons" className="flex flex-col md:flex-row items-center justify-between gap-4 my-6 px-4">
                    ...
                    {isOwner && <DownloadReportButton card={card} cardType="mtg" showFounderEmblem={showFounderEmblem} showVipEmblem={showVipEmblem} showCardLoversEmblem={showCardLoversEmblem} labelStyle={labelStyle} customLabelConfig={activeConfig} />}
          ...
          {(() => {
            ...
            const href = outOfCredits ? '/credits' : '/upload?category=MTG';
        ...
        <EditCardLabelModal
          ...
          onSaved={async () => {
            try {
              const session = getStoredSession();
              const userParam = session?.user?.id ? `&user_id=${session.user.id}` : '';
              const res = await fetch(`/api/mtg/${card.id}?t=${Date.now()}${userParam}`);
              if (res.ok) {
                const data = await res.json();
                setCard(data);
              }
