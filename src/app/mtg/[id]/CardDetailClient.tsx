...
import { generateMTGEbaySearchUrl, generateMTGEbaySoldListingsUrl, type CardData } from "@/lib/ebayUtils";
...
export function MTGCardDetails() {
  ...
  const fetchMTGCardDetails = useCallback(async () => {
    ...
    try {
      ...
      console.log(`[FRONTEND DEBUG] Calling MTG API endpoint: /api/mtg/${cardId}`);
      const res = await fetch(`/api/mtg/${cardId}?t=${Date.now()}${userParam}`); // Cache-busting
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) {
          ...
          const retryWithBackoff = async (attempt: number = 1): Promise<void> => {
            ...
            try {
              console.log(`[FRONTEND DEBUG] Retry ${attempt}: Calling /api/mtg/${cardId}`);
              const retryRes = await fetch(`/api/mtg/${cardId}?t=${Date.now()}${userParam}`); // Cache-busting
  ...
  useEffect(() => {
    if (cardId) {
      console.log('Fetching MTG Card Details for:', cardId);
      fetchMTGCardDetails();
    }
  ...
  const regradeCard = async () => {
    try {
      ...
      const res = await fetch(`/api/mtg/${cardId}?force_regrade=true&t=${Date.now()}`, {
  ...
  // 💰 Refresh price from Scryfall API
  const refreshPrice = async () => {
    if (!card || isRefreshingPrice) return;

    try {
      setIsRefreshingPrice(true);
      setPriceRefreshError(null);

      console.log(`[Price Refresh] Fetching fresh prices for card ${card.id}`);

      const response = await fetch(`/api/mtg/${card.id}/refresh-price`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        }
      ...
      console.log('[Price Refresh] Success:', data);
      ...
      // Also update the card object
      setCard(prev => prev ? {
        ...prev,
        scryfall_price_usd: data.prices.usd,
        scryfall_price_usd_foil: data.prices.usd_foil,
        scryfall_price_eur: data.prices.eur,
        scryfall_price_updated_at: data.updated_at
      } : null);

    } catch (error: any) {
      console.error('[Price Refresh] Error:', error);
      setPriceRefreshError(error.message || 'Failed to refresh prices');
    } finally {
      setIsRefreshingPrice(false);
    }
  };

  // Helper: Calculate staleness of price data
  const getPriceStaleness = (updatedAt: string | null | undefined): { days: number; label: string; color: string } => {
    if (!updatedAt) {
      return { days: -1, label: 'Never updated', color: 'text-gray-500' };
    }
    ...
    if (daysDiff === 0) {
      return { days: 0, label: 'Updated today', color: 'text-green-600' };
    } else if (daysDiff === 1) {
      return { days: 1, label: 'Updated yesterday', color: 'text-green-600' };
    } else if (daysDiff <= 7) {
      return { days: daysDiff, label: `${daysDiff} days ago`, color: 'text-yellow-600' };
    } else if (daysDiff <= 30) {
      return { days: daysDiff, label: `${daysDiff} days ago`, color: 'text-orange-600' };
    } else {
      return { days: daysDiff, label: `${daysDiff} days ago`, color: 'text-red-600' };
    }
  };
  ...
  if (!card) {
    return (
      <div className="text-center p-8">
        <h1 className="text-2xl font-bold mb-4">MTG Card Not Found</h1>
        <Link href="/upload/mtg" className="text-blue-500">
          Back to MTG Upload
        </Link>
      </div>
    );
  ...
  // 🎯 For MTG cards: Build dvgGrading from conversational data
  const dvgGrading = {
    ...
    // 🆕 Always include sub_scores from conversational data for MTG cards
  ...
  // 🎴 MTG CARD INFO: Read from conversational_card_info
  // Following Pokemon pattern: Scryfall API data is merged into card_info by the API route
  ...
  const cardInfo = {
    ...
    manufacturer: stripMarkdown(convInfo.manufacturer) || card.manufacturer_name || 'Wizards of the Coast',
    ...
    // 🎴 MTG-SPECIFIC FIELDS - From conversational_card_info (Scryfall data merged in by API)
    mana_cost: stripMarkdown(convInfo.mana_cost) || card.mana_cost || null,
    color_identity: stripMarkdown(convInfo.color_identity) || card.color_identity || null,
    mtg_card_type: stripMarkdown(convInfo.mtg_card_type) || card.mtg_card_type || null,
    creature_type: stripMarkdown(convInfo.creature_type) || card.creature_type || null,
    power_toughness: stripMarkdown(convInfo.power_toughness) || card.power_toughness || null,
  ...
  // 🎯 MTG cards: Map conversational data to expected structure
  ...
  // For MTG cards, create gradingScale from conversational data
              ...
              <div className="bg-gradient-to-b from-gray-50 to-white border-2 border-purple-600 rounded-lg shadow-md p-3 mb-3 h-[110px]">
                <div className="flex items-center justify-between h-full">
                  ...
                  <div className="flex-1 min-w-0 mx-3 flex flex-col justify-center gap-0.5">
                    {/* Line 1: Card Name - Dynamic Font Sizing */}
                    <div
          ...
          {dvgGrading && Object.keys(dvgGrading).length > 0 && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4 my-6 px-4">
                ...
                <DownloadReportButton card={card} cardType="mtg" />
              ...
              <div className="bg-white rounded-xl shadow-lg border-2 border-gray-200 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {(card.scryfall_id || card.conversational_card_info?.scryfall_id) && (
                    <div>
                      <p className="text-sm font-semibold text-gray-600 mb-1">Database</p>
                      <a
                        href={`https://scryfall.com/card/${card.scryfall_id || card.conversational_card_info?.scryfall_id}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-semibold hover:bg-purple-700 transition-colors shadow-md"
                      >
                        🔍 Scryfall
                      </a>
                    </div>
              ...
              {(() => {
                // Use local prices if available (from refresh), otherwise use card data
                const currentPrices = {
                  usd: localPrices?.usd ?? card.scryfall_price_usd,
                  usd_foil: localPrices?.usd_foil ?? card.scryfall_price_usd_foil,
                  eur: localPrices?.eur ?? card.scryfall_price_eur,
                  updated_at: localPrices?.updated_at ?? card.scryfall_price_updated_at
                };
                const hasPrices = currentPrices.usd || currentPrices.usd_foil || currentPrices.eur;
                const staleness = getPriceStaleness(currentPrices.updated_at);
                ...
                <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
                  ...
                  {priceRefreshError && (
                    <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                      <p className="text-sm text-amber-800">{priceRefreshError}</p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    {currentPrices.usd && (
                      <div className="bg-gray-50 rounded-lg p-4 text-center">
                        ...
                        <p className="text-2xl font-bold text-gray-900">${currentPrices.usd.toFixed(2)}</p>
                      </div>
                    )}

                    {/* USD Price (Foil) */}
                    {currentPrices.usd_foil && (
                      <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-4 text-center border border-purple-200">
                        ...
                        <p className="text-2xl font-bold text-purple-900">${currentPrices.usd_foil.toFixed(2)}</p>
                      </div>
                    )}
                    ...
                    {currentPrices.eur && (
                      <div className="bg-blue-50 rounded-lg p-4 text-center">
                        ...
                        <p className="text-2xl font-bold text-blue-900">€{currentPrices.eur.toFixed(2)}</p>
                      </div>
                    )}

                    {/* Price Updated with Staleness */}
                    <div className="bg-gray-50 rounded-lg p-4 text-center flex flex-col justify-center">
                      <p className="text-xs text-gray-500 mb-1">Last Updated</p>
                      {currentPrices.updated_at ? (
                        <>
                          <p className="text-sm text-gray-700">
                            {new Date(currentPrices.updated_at).toLocaleDateString()}
                          </p>
                          <p className={`text-xs mt-1 ${staleness.color}`}>
                            {staleness.days > 7 && '⚠️ '}{staleness.label}
                          </p>
                        </>
                      ) : (
                  ...
                  <p className="text-xs text-gray-500 mt-3 text-center">
                    Prices from Scryfall daily market data • Click refresh to update
                  </p>
                </div>
              ) : (
                /* No prices available - show option to fetch */
                <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-bold text-gray-800">Market Prices</h2>
                  </div>
                  <div className="text-center py-4">
                    <p className="text-gray-500 mb-3">No pricing data available for this card.</p>
                    <button
                      onClick={refreshPrice}
                      disabled={isRefreshingPrice}
                    ...
                    >
                      {isRefreshingPrice ? (
                        ...
                        <>
                          ...
                          <span>Fetch Prices from Scryfall</span>
                        </>
                      )}
                    </button>
                    {priceRefreshError && (
                      <p className="text-sm text-red-600 mt-2">{priceRefreshError}</p>
                    )}
                  </div>
                </div>
              );
