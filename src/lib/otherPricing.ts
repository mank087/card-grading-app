 ...
 */
async function searchOtherCardPricesUncapped(
  params: OtherCardSearchParams
...
export async function searchOtherCardPrices(
  params: OtherCardSearchParams
): Promise<Awaited<ReturnType<typeof searchOtherCardPricesUncapped>>> {
  const result = await searchOtherCardPricesUncapped(params);
  ...
  if (capped !== result.matchConfidence) {
    console.log(`[OtherPricing] Match confidence capped ${result.matchConfidence} -> ${capped}: the query had no set and no year`);
  }
