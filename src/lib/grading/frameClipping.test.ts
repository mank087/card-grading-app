...
describe('revised shape rule, GRADING_EVIDENCE_V2 only (production photos checked by eye, Sept 20 2026)', () => {
  it('leaves a complete card alone when one corner sits a few units inside the photo', () => {
    ...
    expect(clippedCorners([{ x: 20, y: 25 }, { x: 995, y: 25 }, { x: 985, y: 983 }, { x: 33, y: 987 }], 'front', true)).toEqual([]);
    // Zeraora V: fills the frame, every corner visible; top-left at y=6.
    expect(clippedCorners([{ x: 51, y: 6 }, { x: 973, y: 31 }, { x: 984, y: 972 }, { x: 72, y: 991 }], 'front', true)).toEqual([]);
  });
  ...
  it('still catches a card that runs off one side, even though neither corner clamps exactly', () => {
    ...
    expect(clippedCorners([{ x: 97, y: 194 }, { x: 842, y: 194 }, { x: 894, y: 996 }, { x: 92, y: 996 }], 'front', true))
      .toEqual(['front bottom-right', 'front bottom-left']);
  });

  it('treats a corner within one unit of the border as clamped to it', () => {
    expect(clippedCorners([{ x: 1, y: 40 }, { x: 900, y: 40 }, { x: 900, y: 900 }, { x: 40, y: 900 }], 'back', true)).toEqual(['back top-left']);
    expect(clippedCorners([{ x: 2, y: 40 }, { x: 900, y: 40 }, { x: 900, y: 900 }, { x: 40, y: 900 }], 'back', true)).toEqual([]);
  });

  it('ignores a malformed quad', () => {
    expect(clippedCorners([{ x: 0, y: 0 }, null as any, { x: 5, y: 5 }, { x: 9, y: 9 }], 'front', true)).toEqual([]);
  });
});

describe('the rule in force', () => {
  it('is still the original one: any corner within 0.6% of the border', () => {
    // The revised rule is off by default; see the note on clippedCorners.
    expect(clippedCorners([{ x: 20, y: 25 }, { x: 995, y: 25 }, { x: 985, y: 983 }, { x: 33, y: 987 }], 'front')).toEqual(['front top-right']);
  });
});
