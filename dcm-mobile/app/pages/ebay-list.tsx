...
export default function EbayListScreen() {
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      // 1. Upload images — uses base64 data URLs generated by the hidden WebView
      const imagesToUpload: Record<string, string> = {}
      if (selectedImages.front && imageUrls.front) imagesToUpload.front = imageUrls.front
      if (selectedImages.back && imageUrls.back) imagesToUpload.back = imageUrls.back
      if (selectedImages.miniReport && imageUrls.miniReport) imagesToUpload.miniReport = imageUrls.miniReport
      if (selectedImages.rawFront && imageUrls.rawFront) imagesToUpload.rawFront = imageUrls.rawFront
      if (selectedImages.rawBack && imageUrls.rawBack) imagesToUpload.rawBack = imageUrls.rawBack

      // Fall back to raw signed URLs if generation didn't run / failed
                ...
                {imagesReady && (
                  <View style={st.imageGrid}>
                    {([
                      ...
                      if (!url) return null
                      const selected = (selectedImages as any)[key]
                      return (
                        <TouchableOpacity
                          key={key}
                          style={[st.imageTile, selected && st.imageTileSelected]}
                          onPress={() => setSelectedImages(p => ({ ...p, [key]: !selected }))}
                          activeOpacity={0.8}
                        >
                          <Image source={{ uri: url }} style={st.imageTileThumb} resizeMode="contain" />
                          <View style={st.imageTileFooter}>
                            <Ionicons
                              name={selected ? 'checkmark-circle' : 'ellipse-outline'}
                              size={14}
                              color={selected ? Colors.purple[600] : Colors.gray[400]}
                            />
                            <Text style={st.imageTileLabel} numberOfLines={1}>{label}</Text>
                          </View>
                        </TouchableOpacity>
                      )
                    })}
...
const st = StyleSheet.create({
  ...
  imageTile: { width: '31%', borderRadius: 8, borderWidth: 2, borderColor: Colors.gray[200], backgroundColor: '#fff', overflow: 'hidden' },
